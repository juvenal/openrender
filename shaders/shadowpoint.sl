...
light shadowpoint (float intensity = 1;
    ...
    if (shadowname_px == "" || shadowname_nx == "" ||
        ...
        illuminate (from) {
            ...
            if ((ax > ay) && (ax > az)) {
                if (xcomp(Lworld) > 0.0) {
                    atten = shadow(shadowname_px, Ps);
                }
                else {
                    atten = shadow(shadowname_nx, Ps);
                }
            }
            else if ((ay > ax) && (ay > az)) {
                if (ycomp(Lworld) > 0.0) {
                    atten = shadow(shadowname_py, Ps);
                }
                else {
                    atten = shadow(shadowname_ny, Ps);
                }
            }
            else if ((az > ay) && (az > ax)) {
                if (zcomp(Lworld) > 0.0) {
                    atten = shadow(shadowname_pz, Ps);
                }
                else {
                    atten = shadow(shadowname_nz, Ps);
                }
            }
