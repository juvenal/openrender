 ...
 *       Entropy/BMRT area lights for the time being).
 *
 ...
 *       corresponds to a cosine falloff.  For an area light, the
 ...
 *   shadowbias - the amount of shadow bias to add to the lookup.  For
 *       Entropy, a shadowbias value < 0 indicates to use the global default
 *       (set by Option "shadow" "bias").
 *   shadowsamples - the number of samples to use.
 *
 * Ray-traced shadows.  For Entropy, ray-traced shadows are achieved by
 * passing shadowname = "shadow".  
 *
 ...
 *       diffuse(), specular() and phong() all do this, but if you write
 *       your own illuminance loops in your surface shader, you've got
 *       to account for it yourself.  The ARMan book explains how
 *       to do this.
 ...
 *       __nonspecular.
 *   __foglight - many volumetric or smoke shaders will add
 ...
 *
 *   Coded in SL by Larry Gritz, published in SIGGRAPH '98 course notes #11,
 *   revised and published in "Advanced RenderMan: CGI for Motion Pictures"
 *   (Apodaca & Gritz, Morgan-Kaufmann 1999).
 *
 *   Somewhat revised for inclusion in Exluna's Entropy product, 2001.
 *   
 * Entropy/BMRT are :
 * (c) Copyright 2001 Exluna, Inc. All rights reserved.
 *
 * $Revision: 1.14.2.1 $    $Date: 2002/04/18 20:23:03 $
 *
 ****************************************************************************/
...
#ifdef EXLUNA
    /* If it's an area light, we want the point and normal of the
     * light geometry.  If not an area light, Entropy/BMRT guarantees
     * P,N will be the origin and z-axis of shader space.
     */
    point from = P;
    vector axis = normalize(N);
#else
    /* For renderers without area lights, we've gotta do it the hard way */
