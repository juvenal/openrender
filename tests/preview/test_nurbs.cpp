...
static float deBoorBasis(const float *knots, int i, int k, float t) {
    ...
    if (d1 > 1e-8f)
        c1 = (t - knots[i]) / d1 * deBoorBasis(knots, i, k - 1, t);
    if (d2 > 1e-8f)
        c2 = (knots[i + k] - t) / d2 * deBoorBasis(knots, i + 1, k - 1, t);
    return c1 + c2;
}
...
static std::vector<float3> tessNURBS(
    int nu, int nv, int uorder, int vorder, const float *uknots, const float *vknots, float umin, float umax, float vmin, float vmax, const float3 *ctrl) {
    const int N = 12;
...
int main() {
    ...
    bool inBounds = true;
    for (auto &v : edges) {
        if (v.x < -0.01f || v.x > 1.01f || v.y < -0.01f || v.y > 1.01f) {
            inBounds = false;
            break;
        }
    }
