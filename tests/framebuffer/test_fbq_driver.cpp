/**
 * tests/framebuffer/test_fbq_driver.cpp
 *
 * Unit tests for CQDisplay (macOS IPC client) — verifies the TLV packet
 * encoding functions from fbipc.h that CQDisplay uses.
 *
...
// ---------------------------------------------------------------------------
// T011: START packet construction
// ---------------------------------------------------------------------------

static void test_start_packet_field_values() {
    int sv[2]; makePair(sv);
    int writerFd = sv[0], readerFd = sv[1];

    bool ok = sendStart(writerFd, 1920, 1080, 3, "test-scene");
    EXPECT_TRUE(ok);
    close(writerFd);

    // Read and verify header
    FBHeader hdr;
    EXPECT_TRUE(readAll(readerFd, &hdr, sizeof(hdr)));
    ...
    FBStartPayload sp;
    EXPECT_TRUE(readAll(readerFd, &sp, sizeof(sp)));
    ...
    char title[11] = {};
    EXPECT_TRUE(readAll(readerFd, title, 10));
...
// ---------------------------------------------------------------------------
// T011: Socket-closed mid-render → writeAll returns false (driver can detect)
// ---------------------------------------------------------------------------
...
static void test_write_to_closed_socket_fails() {
    ...
    signal(SIGPIPE, SIG_IGN); // don't crash on EPIPE

    float dummy[3] = {0.1f, 0.2f, 0.3f};
    bool ok = sendData(sv[0], 0, 0, 1, 1, 3, dummy);
    EXPECT_EQ(ok, false); // write to half-closed socket must fail
