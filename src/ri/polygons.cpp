 ...
 *
 * Description:
 *   This file implements the functionality for polygons.
 *
 * Authors:
...
void CPolygonTriangle::intersect(CShadingContext *context, CRay *cRay) {
    ...
    if ((vertices = pl->data1) != NULL) {
        interpolatev(t0, vert0, vertices + this->v0 * 3, cRay->time);
        vert0 = t0;
        interpolatev(t1, vert1, vertices + this->v1 * 3, cRay->time);
        vert1 = t1;
        interpolatev(t2, vert2, vertices + this->v2 * 3, cRay->time);
        vert2 = t2;
    }
...
void CPolygonQuad::intersect(CShadingContext *context, CRay *cRay) {
    ...
    if ((vertices = pl->data1) != NULL) {
        interpolatev(t0, P00, vertices + this->v0 * 3, cRay->time);
        P00 = t0;
        interpolatev(t1, P10, vertices + this->v1 * 3, cRay->time);
        P10 = t1;
        interpolatev(t2, P01, vertices + this->v2 * 3, cRay->time);
        P01 = t2;
        interpolatev(t3, P11, vertices + this->v3 * 3, cRay->time);
        P11 = t3;
    }
...
CPolygonMesh::CPolygonMesh(CAttributes *a, CXform *x, CPl *pl, int npoly, int *nholes, int *nvertices, int *vertices) : CObject(a, x) {
    ...
    this->nholes = new int[npoly];
    memcpy(this->nholes, nholes, npoly * sizeof(int));
    this->nvertices = new int[nloops];
    memcpy(this->nvertices, nvertices, nloops * sizeof(int));
    this->vertices = new int[nverts];
...
inline void createTriangle(const int *vindices, const int vi0, const int vi1, const int vi2, CMeshData &data) {
    ...
    // Create the triangle
    cTriangle = new CPolygonTriangle(data.meshAttributes, data.meshXform, data.mesh, vindices[vi0], vindices[vi1], vindices[vi2], data.meshFacevaryingNumber + vi0, data.meshFacevaryingNumber + vi1, data.meshFacevaryingNumber + vi2, data.meshUniformNumber);
    ...
    do {
        ...
        if ((sVertex->xy != to->xy) &&
            ...
            if (area(b[0], b[1], c[0], c[1], s1[0], s1[1]) *
                    area(b[0], b[1], c[0], c[1], s2[0], s2[1]) <=
                0) {

                if (area(s1[0], s1[1], s2[0], s2[1], b[0], b[1]) *
                        area(s1[0], s1[1], s2[0], s2[1], c[0], c[1]) <=
                    0) {
...
static inline int orientationCheck(CTriVertex *loop, int cw, CMeshData &data) {
    ...
    for (cVertex = minVertex->next; cVertex != loop; cVertex = cVertex->next, numVertices++) {
        ...
        if (cVertex->xy[1] > minVertex->xy[1])
            minVertex = cVertex;
    }
