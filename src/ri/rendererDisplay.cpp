 ...
 *
 * Description:
 *   This file implements the functionality for rendererDisplay.
 *
 * Authors:
...
void CRenderer::endDisplays() {
    ...
    for (i = 0; i < numDisplays; i++) {
        ...
        if (datas[i].displayName != NULL)
            free(datas[i].displayName);

        // Delete the fill array if set
        int j;
        for (j = 0; j < datas[i].numChannels; j++) {
            if (datas[i].channels[j].fill != NULL)
                delete[] datas[i].channels[j].fill;
        }
    ...
    if (sampleOrder != NULL)
        delete[] sampleOrder;
    if (sampleDefaults != NULL)
        delete[] sampleDefaults;
    if (compChannelOrder != NULL)
        delete[] compChannelOrder;
    if (nonCompChannelOrder != NULL)
        delete[] nonCompChannelOrder;
...
void CRenderer::dispatch(int left, int top, int width, int height, float *pixels) {
    ...
    for (i = 0; i < numDisplays; i++) {
        if (datas[i].module != NULL) {
            ...
            if (size < MAX_DISPATCH_SIZE)
                dispatchData = (float *)alloca(size);
            else
                dispatchData = new float[width * height * imageSamples];
...
void CRenderer::clear(int left, int top, int width, int height) {
    ...
    if (size < MAX_DISPATCH_SIZE)
        pixels = (float *)alloca(size);
    else
        pixels = new float[width * height * numSamples];

    for (int i = 0; i < width * height * numSamples; i++)
        pixels[i] = 0;
...
void CRenderer::commit(int left, int top, int xpixels, int ypixels, float *pixels) {
    ...
    if (flags & OPTIONS_FLAGS_PROGRESS) {
        ...
        if (numRenderedBuckets == xBuckets * yBuckets)
            info(CODE_PROGRESS, "Done                    \r\n");
        else
            info(CODE_PROGRESS, "Done %%%3.2f\r", stats.progress);
    }
    ...
    if ((top == 0) && (left == 0)) {
        if (renderTop > 0)
            clear(0, 0, xres, renderTop);
    }

    if (left == 0) {
        if (renderLeft > 0)
            clear(0, top + renderTop, renderLeft, ypixels);
    }

    if ((left + xpixels) == xPixels) {
        if (renderRight < xres)
            clear(renderRight, top + renderTop, xres - renderRight, ypixels);
    }

    if (((top + ypixels) == yPixels) && ((left + xpixels) == xPixels)) {
        if (renderBottom < yres)
            clear(0, renderBottom, xres, yres - renderBottom);
    }
...
void CRenderer::computeDisplayData() {
    ...
    for (cDisplay = displays; cDisplay != NULL; cDisplay = cDisplay->next) {
        ...
        do {
            ...
            if (nextComma != NULL) {
                for (tmp = nextComma - 1; isspace(*tmp) && (tmp > sampleName); tmp--)
                    *tmp = '\0';
                *nextComma++ = '\0';
                while (isspace(*nextComma))
                    nextComma++;
            }
            while (isspace(*sampleName))
                sampleName++;
        ...
        if (strcmp(outDevice, "shadow") == 0 || strcmp(outDevice, "zfile") == 0 || strcmp(outDevice, "tiff") == 0 || strcmp(outDevice, "png") == 0)
            outDevice = RI_FILE;
    ...
    for (i = 0, k = 0; i < numDisplays; i++) {
        for (j = 0; j < datas[i].numChannels; j++) {
            ...
            if (datas[i].channels[j].outType == -1)
                continue;
