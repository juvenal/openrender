 ...
 *
 * Description:
 *   This file defines the interface for scriptFunctions.
 *
 * Authors:
...
#define FUN5OUTEXPR_PRE                 \
    float *res, *op1, *op2, *op3, *op4; \
    operand(0, res, float *);           \
    operand(1, op1, float *);           \
    operand(2, op2, float *);           \
    operand(3, op3, float *);           \
...
#define MOD2EXP                                     \
    *res = (float)fmod((double)*op1, (double)*op2); \
    if (*res < 0)                                   \
        *res += *op2;                               \
    assert(*res >= 0);
DEFFUNC(Mod2, "mod", "f=ff", FUN3EXPR_PRE, MOD2EXP, FUN3EXPR_UPDATE(1, 1, 1), NULL_EXPR, 0)

#define CLAMPFEXP *res = (*op1 < *op2 ? *op2 : *op1 > *op3 ? *op3 \
                                                           : *op1);
DEFFUNC(Clampf, "clamp", "f=fff", FUN4EXPR_PRE, CLAMPFEXP, FUN4EXPR_UPDATE(1, 1, 1, 1), NULL_EXPR, 0)

#define CLAMPVEXP                                                   \
    res[0] = (op1[0] < op2[0] ? op2[0] : op1[0] > op3[0] ? op3[0]   \
                                                         : op1[0]); \
    res[1] = (op1[1] < op2[1] ? op2[1] : op1[1] > op3[1] ? op3[1]   \
                                                         : op1[1]); \
    res[2] = (op1[2] < op2[2] ? op2[2] : op1[2] > op3[2] ? op3[2]   \
                                                         : op1[2]);
...
#define MIXVEXP                                                                  \
    res[0] = (float)(op1[0] * (1.0 - (double)op3[0]) + op2[0] * (double)op3[0]); \
...
#define PTLINEDEXP_PRE         \
    FUN4EXPR_PRE               \
    vector vtmp, vtmp2, vtmp3; \
    float l;

#define PTLINEDEXP                                                     \
    const float *ftmp = op1;                                           \
...
#define FORMATEXPR                       \
    PRINTEXPR(output, strArg, opf, ops); \
    savestring(*res, output);

#define FORMAT_UPDATE                        \
    res += resStep;                          \
    strArg += strStep;                       \
    for (int i = 0; i < numArguments; ++i) { \
        opf[i] += opSteps[i];                \
