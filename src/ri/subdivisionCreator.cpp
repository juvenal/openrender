 ...
 *
 * Description:
 *   This file implements the functionality for subdivisionCreator.
 *
 * Authors:
...
class CSFace {
    public:
        ...
        void create(CObject *&children) {
            ...
            if (hole == TRUE)
                return; // If we're a hole, don't create
            ...
            if (numEdges != 4) {
                ...
                if (numExtraordinary == 1) {
                    if (vertices[extraordinary]->valence < 3)
                        funny = TRUE;
                }
            }
            ...
            if ((split == FALSE) || (funny == TRUE)) {
                ...
                if (funny == FALSE) {
                    ...
                    if (funnyBorder == TRUE) {
                        ...
                        if (data.irregularVertices[0 * (nv + 2) + 0] == NULL)
                            data.irregularVertices[0 * (nv + 2) + 0] = data.irregularVertices[1 * (nv + 2) + 1];
                        if (data.irregularVertices[0 * (nv + 2) + (nv + 1)] == NULL)
                            data.irregularVertices[0 * (nv + 2) + (nv + 1)] = data.irregularVertices[1 * (nv + 2) + (nv)];
                        if (data.irregularVertices[(nv + 1) * (nv + 2) + (nv + 1)] == NULL)
                            data.irregularVertices[(nv + 1) * (nv + 2) + (nv + 1)] = data.irregularVertices[(nv) * (nv + 2) + (nv)];
                        if (data.irregularVertices[(nv + 1) * (nv + 2) + 0] == NULL)
                            data.irregularVertices[(nv + 1) * (nv + 2) + 0] = data.irregularVertices[(nv) * (nv + 2) + 1];
...
void CSVertex::sort(CSVertex **v, CSEdge *cEdge, CSFace *cFace, int exp) {
    ...
    do {
        ...
        if (!((cEdge->vertices[0] == this) || (cEdge->vertices[1] == this)))
            cEdge = cFace->edges[(i + 3) & 3];

        if (cEdge->faces[0] == cFace)
            cFace = cEdge->faces[1];
        else
            cFace = cEdge->faces[0];
...
void CSVertex::compute() {
    ...
    if (parentv != NULL)
        ...
        parente->compute(vertex);
    else if (parentf != NULL)
        parentf->compute(vertex);
    else {
        assert(FALSE);
    }
}
...
void CSVertex::compute(float *vertex) {
    ...
    if ((numSharp > 2) || (valence == 2)) { // We're a corner vertex
        ...
        for (cEdge = edges; cEdge != NULL; cEdge = cEdge->next) {
            ...
            if (cEdge->edge->vertices[0] == this)
                oVertex = cEdge->edge->vertices[1];
            else {
                assert(cEdge->edge->vertices[1] == this);
                oVertex = cEdge->edge->vertices[0];
            }

            if (oVertex->vertex == NULL)
                oVertex->compute();
...
CSubdivMesh::CSubdivMesh(CAttributes *a, CXform *x, CPl *c, int numFaces, int *numVerticesPerFace, int *vertexIndices, int ntags, const char **tags, int *nargs, int *intargs, float *floatargs) : CObject(a, x) {
    ...
    this->numVerticesPerFace = new int[numFaces];
    memcpy(this->numVerticesPerFace, numVerticesPerFace, sizeof(int) * numFaces);
    this->vertexIndices = new int[j];
    memcpy(this->vertexIndices, vertexIndices, sizeof(int) * j);
    this->ntags = ntags;
    this->tags = NULL;
    this->nargs = NULL;
    this->intargs = NULL;
    ...
    if (ntags > 0) {
        ...
        this->nargs = new int[ntags * 2];
        memcpy(this->nargs, nargs, sizeof(int) * ntags * 2);
        ...
        if (ias > 0)
            this->intargs = new int[ias];
        memcpy(this->intargs, intargs, ias * sizeof(int));
        if (fas > 0)
            this->floatargs = new float[fas];
        memcpy(this->floatargs, floatargs, fas * sizeof(float));
    }
...
void CSubdivMesh::create(CShadingContext *context) {
    ...
    for (i = 0, cnargs = nargs, cintargs = intargs, cfloatargs = floatargs; i < ntags; i++) {
        if (strcmp(tags[i], RI_HOLE) == 0) {
            if (cnargs[1] != 0)
                error(CODE_RANGE, "Hole takes no floating point arguments\n");
