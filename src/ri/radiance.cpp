 ...
 *
 * Description:
 *   This file implements the functionality for radiance.
 *
 * Authors:
    ...
    public:
        ...
        void lookup(float *C, const float *P, const float *N) {
            ...
            for (int ii = 1; ii <= numFound; ii++) {
                ...
                if (dotvv(D, cTriangle->N) <= 0.001)
                    continue; // The query point is behind the triangle

                if (dotvv(cTriangle->N, N) >= (float)0.99)
                    continue; // The triangle can not have a big form factor
                ...
                if ((cornerStat[0] | cornerStat[1] | cornerStat[2]) == 0)
                    continue;
                ...
                if (cornerStat[0] & cornerStat[1] & cornerStat[2]) {
                    // No
                    numCorners = 3;
                    subvv(corners + 0, cTriangle->corners + 0, P);
                    normalizev(corners + 0);
                    subvv(corners + 3, cTriangle->corners + 3, P);
...
void CRadianceCache::finalize(const CGlobalIllumLookup *l) {
    ...
    if (numShadingPoints > 0) {
        ...
        for (i = 0, cSample = shadingPoints; cSample != NULL; cSample = cSample->next)
            samples[i++] = cSample;
                ...
                *stack++ = root;
                while (stack > stackBase) {
                    ...
                    for (cHarmonic = cNode->samples; cHarmonic != NULL; cHarmonic = cHarmonic->next) {
                        ...
                        // Are we in the range ?
                        if ((nominator = dotvv(D, D)) > (cHarmonic->dP * cHarmonic->dP))
                            continue;
...
void CRadianceCache::prepareTriangles() {
    ...
    for (i = numTriangles; i > 0; i--) {
        ...
        crossvv(N, D1, D2);
        if (dotvv(N, N) == 0)
            continue; // Skip the degenerate triangles
...
int CRadianceCache::partition(int numRays, CShadingPoint **rays) {
    ...
    if (rays[last]->next == (CShadingPoint *)1)
        last++;

#ifdef _DEBUG
    // Sanity check here
    for (i = 0; i < last; i++)
        assert(rays[i]->next == (CShadingPoint *)1);
    for (; i < numRays; i++)
        assert(rays[i]->next == (CShadingPoint *)2);
#endif
