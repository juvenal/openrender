 ...
 *
 * Description:
 *   This file implements the functionality for noise.
 *
 * Authors:
...
T grad(int hash, T x, T y, T z) {
    int h = hash & 15;
    const T u = h < 8 ? x : y;
    const T v = h < 4 ? y : h == 12 || h == 14 ? x
                                               : z;
    return ((h & 1) ? -u : u) + ((h & 2) ? -v : v);
}

template <class T>
T grad(int hash, T x, T y, T z, T t) {
    int h = hash & 31;
    const T u = h < 24 ? x : y;
    const T v = h < 16 ? y : z;
    const T w = h < 8 ? z : t;
...
template <class T>
T noise(T x, const unsigned char *perm) {
    int ix0, ix1;
    T fx0, fx1;
    T s, n0, n1;
...
template <class T>
T pnoise(T x, int px, const unsigned char *perm) {
    int ix0, ix1;
    T fx0, fx1;
    T s, n0, n1;
...
template <class T>
T noise(T x, T y, const unsigned char *perm) {
    int ix0, iy0, ix1, iy1;
    T fx0, fy0, fx1, fy1;
    T s, t, nx0, nx1, n0, n1;
...
template <class T>
T pnoise(T x, T y, int px, int py, const unsigned char *perm) {
    int ix0, iy0, ix1, iy1;
    T fx0, fy0, fx1, fy1;
    T s, t, nx0, nx1, n0, n1;
...
template <class T>
T noise(T x, T y, T z, const unsigned char *perm) {
...
template <class T>
T pnoise(T x, T y, T z, int px, int py, int pz, const unsigned char *perm) {
    int ix0, iy0, ix1, iy1, iz0, iz1;
    T fx0, fy0, fz0, fx1, fy1, fz1;
    T s, t, r;
    T nxy0, nxy1, nx0, nx1, n0, n1;
...
template <class T>
T noise(T x, T y, T z, T w, const unsigned char *perm) {
...
template <class T>
T pnoise(T x, T y, T z, float w, int px, int py, int pz, int pw, const unsigned char *perm) {
    int ix0, iy0, iz0, iw0, ix1, iy1, iz1, iw1;
    T fx0, fy0, fz0, fw0, fx1, fy1, fz1, fw1;
    T s, t, r, q;
    T nxyz0, nxyz1, nxy0, nxy1, nx0, nx1, n0, n1;
...
#define permute(__val)                                       \
    if (__val < 0)                                           \
        i = permN[(i + ((unsigned int)(__val - 1))) & 4095]; \
    else                                                     \
        i = permN[(i + ((unsigned int)__val)) & 4095];
