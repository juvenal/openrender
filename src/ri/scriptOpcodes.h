 ...
 *
 * Description:
 *   This file defines the interface for scriptOpcodes.
 *
 * Authors:
...
#define FORBEGIN3EXPR_PRE                         \
    const int bodyIndex = argument(0);            \
...
#define MCMPEXPR                                \
    *res = 1.0f;                                \
    for (int index = 0; index < 16; ++index)    \
        if (!(op1[index] OPERATION op2[index])) \
            *res = 0;
...
#define VARITMETICEXPR                \
    res[0] = op1[0] OPERATION op2[0]; \
    res[1] = op1[1] OPERATION op2[1]; \
    res[2] = op1[2] OPERATION op2[2];

#define MARITMETICEXPR                   \
    res[0] = op1[0] OPERATION op2[0];    \
    res[1] = op1[1] OPERATION op2[1];    \
    res[2] = op1[2] OPERATION op2[2];    \
