 ...
 *
 * Description:
 *   This file implements the functionality for pointHierarchy.
 *
 * Authors:
...
CPointHierarchy::CPointHierarchy(const char *n, const float *from, const float *to, FILE *in) : CMap<CPointCloudPoint>(), CTexture3d(n, from, to) {
    ...
    for (i = 0; i < numChannels; i++) {
        if ((strcmp(channels[i].name, areaName) == 0) && (channels[i].numSamples == 1))
            ...
            radiosityIndex = channels[i].sampleStart;
    }
...
int CPointHierarchy::cluster(int numItems, int *indices) {
    ...
    if (numItems == 1) {
        ...
        if (numItems >= ALLOCA_MAX_ITEMS)
            membership = new int[numItems * 2];
        else
            membership = (int *)alloca(numItems * 2 * sizeof(int));
    ...
    *stack++ = 0;
    while (stack > stackBase) {
        ...
        if (currentNode < 0) {
            ...
            if (dotvv(P, item->N) <= dotvv(item->P, item->N))
                continue;
            ...
            const float *src = data.array + item->entryNumber;
            if (areaIndex == -1)
                form = ff(P, N, item->P, item->N, item->dP);
            else
                form = ff(P, N, item->P, item->N, sqrtf(src[areaIndex] / (float)C_PI));
            assert(form >= 0);
