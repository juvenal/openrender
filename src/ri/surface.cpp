 ...
 *
 * Description:
 *   This file implements the functionality for surface.
 *
 * Authors:
...
void CPatch::dice(CShadingContext *r) {
    ...
    if ((udiv == -1) && (vdiv == -1)) {
        ...
        for (int numTries = 0; numTries < 5; ++numTries) {
            ...
            if (((float)(ustart + ustep)) <= ((float)ustart))
                return;
            if (((float)(vstart + vstep)) <= ((float)vstart))
                return;
...
// FIXME: correct this
            ...
            // Frustrum culling
            if (CRenderer::inFrustrum(bmin, bmax) == FALSE)
                return;
...
CTesselationPatch::CPurgableTesselation *CTesselationPatch::tesselate(CShadingContext *context, char rdiv, int estimateOnly) {
    ...
    int div = rdiv;
    if (div == 1)
        div = 2; // sample() cannot do 1x1, so we do 2x2 and drop down
...
#ifdef DEBUG_STATS
    for (int i = 0; i < TESSELATION_NUM_LEVELS * DEBUG_STATS; i++)
        tessPerLevel[i] = 0;
    for (int i = 0; i < DEBUG_STATS; i++)
        tessPerDepth[i] = 0;
#endif
}
...
void CTesselationPatch::purgeTesselations(CShadingContext *context, CTesselationPatch *entry, int thread, int level, int all) {
    ...
    if (i > 1)
        tesselationQuickSort(activeTesselations, 0, i - 1, thread);

    // Free the memory
    if (all)
        tesselationMaxMemory[level] = 0;
