...
enum class DepthFilterMode { Min,
                             Max,
                             Avg,
                             Mid };
...
class CCompositor {
    public:
        ...
        static bool compositeNonComp(float *dst, const CompositeSample &sample, const vector zvisibilityThreshold, bool pixelHasMatte);
        ...
        static float evaluateDepth(const float *zs, const float *opacities, int count, DepthFilterMode mode, const vector zvisibilityThreshold, bool pixelHasMatte, float zold = -1e30f);
};
