 ...
 *
 * Description:
 *   This file implements the functionality for ri.
 *
 * Authors:
...
RtToken RI_BLOBBY = "blobby";
RtToken RI_BLOBBYTOLERANCE = "tolerance";
RtToken RI_BLOBBYOPCODEORDER = "opcodeorder";
...
const unsigned int RENDERMAN_ALL_BLOCKS = RENDERMAN_BLOCK | RENDERMAN_XFORM_BLOCK | RENDERMAN_ATTRIBUTE_BLOCK | RENDERMAN_WORLD_BLOCK | RENDERMAN_FRAME_BLOCK | RENDERMAN_OBJECT_BLOCK | RENDERMAN_MOTION_BLOCK | RENDERMAN_SOLID_PRIMITIVE_BLOCK | RENDERMAN_SOLID_INTERSECTION_BLOCK | RENDERMAN_SOLID_DIFFERENCE_BLOCK | RENDERMAN_SOLID_UNION_BLOCK | RENDERMAN_RESOURCE_BLOCK | RENDERMAN_ARCHIVE_BLOCK;
...
static inline int check(const char *fun, int scope) {
    if (ignoreFrame | ignoreCommand)
        return TRUE;

    if (scope & currentBlock & allowedCommands)
        return FALSE;
...
static inline int extract(char *dest, const char *tag, const char *src) {
    ...
    if ((tmp = strstr(src, tag)) != NULL) {
        if ((tmpEnd = strchr(tmp + 1, ' ')) == NULL)
            tmpEnd = tmp + strlen(tmp);
...
RiDeclare(const char *name, const char *declaration) {
    if (check("RiDeclare", RENDERMAN_ALL_BLOCKS))
        return RI_NULL;
...
RiGetContext(void) {
    if (check("RiGetContext", RENDERMAN_BLOCK))
        return RI_NULL;
...
RiContext(RtContextHandle handle) {
    if (check("RiContext", RENDERMAN_BLOCK))
        return;
...
RiEnd(void) {
    ...
    if (check("RiEnd", RENDERMAN_BLOCK))
        return;
...
RiFrameBegin(RtInt number) {
    if (check("RiFrameBegin", RENDERMAN_BLOCK | RENDERMAN_ARCHIVE_BLOCK))
        return;

    if (frameRangeActive) {
        if ((number < frameBegin) || (number > frameEnd))
            ignoreFrame = TRUE;
        else if (frameStep > 1) {
            if (((number - frameBegin) % frameStep) != 0)
                ignoreFrame = TRUE;
        }
    }
...
RiFrameEnd(void) {
    ...
    if (check("RiFrameEnd", RENDERMAN_FRAME_BLOCK))
        return;
    ...
    if (allowedCommands == RENDERMAN_FRAME_BLOCK)
        allowedCommands = RENDERMAN_ALL_BLOCKS;
}

// WorldBegin - End stuff
EXTERN(RtVoid)
RiWorldBegin(void) {
    if (check("RiWorldBegin", RENDERMAN_BLOCK | RENDERMAN_FRAME_BLOCK | RENDERMAN_ARCHIVE_BLOCK))
        return;
...
RiWorldEnd(void) {
    ...
    if (check("RiWorldEnd", RENDERMAN_WORLD_BLOCK))
        return;
...
RiFormat(RtInt xres, RtInt yres, RtFloat aspect) {
    if (check("RiFormat", VALID_OPTION_BLOCKS))
        return;
...
RiFrameAspectRatio(RtFloat aspect) {
    if (check("RiFrameAspectRatio", VALID_OPTION_BLOCKS))
        return;
...
RiScreenWindow(RtFloat left, RtFloat right, RtFloat bot, RtFloat top) {
    if (check("RiScreenWindow", VALID_OPTION_BLOCKS))
        return;
...
RiCropWindow(RtFloat xmin, RtFloat xmax, RtFloat ymin, RtFloat ymax) {
    if (check("RiCropWindow", VALID_OPTION_BLOCKS))
        return;
...
RiProjectionV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiProjection", VALID_OPTION_BLOCKS))
        return;
...
RiClipping(RtFloat hither, RtFloat yon) {
    if (check("RiClipping", VALID_OPTION_BLOCKS))
        return;
...
RiClippingPlane(RtFloat x, RtFloat y, RtFloat z, RtFloat nx, RtFloat ny, RtFloat nz) {
    if (check("RiClippingPlane", VALID_OPTION_BLOCKS))
        return;
...
RiDepthOfField(RtFloat fstop, RtFloat focallength, RtFloat focaldistance) {
    if (check("RiDepthOfField", VALID_OPTION_BLOCKS))
        return;
...
RiShutter(RtFloat smin, RtFloat smax) {
    if (check("RiShutter", VALID_OPTION_BLOCKS))
        return;
...
RiPixelVariance(RtFloat variation) {
    if (check("RiPixelVariance", VALID_OPTION_BLOCKS))
        return;
...
RiPixelSamples(RtFloat xsamples, RtFloat ysamples) {
    if (check("RiPixelSamples", VALID_OPTION_BLOCKS))
        return;
...
RiPixelFilter(RtFilterFunc function, RtFloat xwidth, RtFloat ywidth) {
    if (check("RiPixelFilter", VALID_OPTION_BLOCKS))
        return;
...
RiExposure(RtFloat gain, RtFloat gamma) {
    if (check("RiExposure", VALID_OPTION_BLOCKS))
        return;
...
RiImagerV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiImager", VALID_OPTION_BLOCKS))
        return;
...
RiQuantize(RtToken type, RtInt one, RtInt qmin, RtInt qmax, RtFloat ampl) {
    if (check("RiQuantize", VALID_OPTION_BLOCKS))
        return;
...
RiDisplayChannelV(RtToken channel, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiDisplayChannel", VALID_OPTION_BLOCKS))
        return;
...
// Step-filter functions (RiGaussianStepFilter, RiCatmullRomStepFilter,
// RiMitchellStepFilter, RiTriangleStepFilter, RiBoxStepFilter) moved to
// src/common/rslConstants.cpp: shaderPl.cpp (libshader_shading) references
// RiCatmullRomStepFilter directly, so it must live where it links even into
// binaries (e.g. sloinfo) that don't link libri.
// They are declared extern in ri.h and linked via openrendercommon.
...
RiHiderV(RtToken type, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiHider", VALID_OPTION_BLOCKS))
        return;
...
RiColorSamples(RtInt N, RtFloat *nRGB, RtFloat *RGBn) {
    if (check("RiColorSamples", VALID_OPTION_BLOCKS))
        return;
...
RiRelativeDetail(RtFloat relativedetail) {
    if (check("RiRelativeDetail", VALID_OPTION_BLOCKS))
        return;
...
RiOptionV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiOption", VALID_OPTION_BLOCKS))
        return;
...
RiAttributeBegin(void) {
    if (check("RiAttributeBegin", RENDERMAN_BLOCK | RENDERMAN_FRAME_BLOCK | RENDERMAN_WORLD_BLOCK | RENDERMAN_ATTRIBUTE_BLOCK | RENDERMAN_XFORM_BLOCK | RENDERMAN_SOLID_PRIMITIVE_BLOCK | RENDERMAN_RESOURCE_BLOCK | RENDERMAN_ARCHIVE_BLOCK))
        return;
...
EXTERN(RtVoid)
RiAttributeEnd(void) {
    if (check("RiAttributeEnd", RENDERMAN_ATTRIBUTE_BLOCK))
        return;
...
EXTERN(RtVoid)
RiColor(RtColor Cs) {
    if (check("RiColor", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiOpacity(RtColor Cs) {
    if (check("RiOpacity", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiTextureCoordinates(RtFloat s1, RtFloat t1, RtFloat s2, RtFloat t2, RtFloat s3, RtFloat t3, RtFloat s4, RtFloat t4) {
    if (check("RiTextureCoordinates", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiLightSourceV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiLightSource", VALID_ATTRIBUTE_BLOCKS))
        return NULL;
...
RiAreaLightSourceV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiAreaLightSource", VALID_ATTRIBUTE_BLOCKS))
        return NULL;
...
RiIlluminate(RtLightHandle light, RtBoolean onoff) {
    if (check("RiIlluminate", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiSurfaceV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiSurface", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiAtmosphereV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiAtmosphere", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiInteriorV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiInterior", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiExteriorV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiExterior", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiShadingRate(RtFloat size) {
    if (check("RiShadingRate", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiShadingInterpolation(RtToken type) {
    if (check("RiShadingInterpolation", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiMatte(RtBoolean onoff) {
    if (check("RiMatte", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiBound(RtBound bound) {
    if (check("RiBound", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiDetail(RtBound bound) {
    if (check("RiDetail", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiDetailRange(RtFloat minvis, RtFloat lowtran, RtFloat uptran, RtFloat maxvis) {
    if (check("RiDetailRange", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiGeometricApproximation(RtToken type, RtFloat value) {
    if (check("RiGeometricApproximation", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiGeometricRepresentation(RtToken type) {
    if (check("RiGeometricRepresentation", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiOrientation(RtToken orientation) {
    if (check("RiOrientation", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiReverseOrientation(void) {
    if (check("RiReverseOrientation", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiSides(RtInt nsides) {
    if (check("RiSides", VALID_ATTRIBUTE_BLOCKS))
        return;
...
EXTERN(RtVoid)
RiIdentity(void) {
    if (check("RiIdentity", VALID_XFORM_BLOCKS))
        return;
...
RiTransform(RtMatrix transform) {
    if (check("RiTransform", VALID_XFORM_BLOCKS))
        return;
...
RiConcatTransform(RtMatrix transform) {
    if (check("RiConcatTransform", VALID_XFORM_BLOCKS))
        return;
...
RiPerspective(RtFloat fov) {
    if (check("RiPerspective", VALID_XFORM_BLOCKS))
        return;
...
RiTranslate(RtFloat dx, RtFloat dy, RtFloat dz) {
    if (check("RiTranslate", VALID_XFORM_BLOCKS))
        return;
...
RiRotate(RtFloat angle, RtFloat dx, RtFloat dy, RtFloat dz) {
    if (check("RiRotate", VALID_XFORM_BLOCKS))
        return;
...
RiScale(RtFloat dx, RtFloat dy, RtFloat dz) {
    if (check("RiScale", VALID_XFORM_BLOCKS))
        return;
...
RiSkew(RtFloat angle, RtFloat dx1, RtFloat dy1, RtFloat dz1, RtFloat dx2, RtFloat dy2, RtFloat dz2) {
    if (check("RiSkew", VALID_XFORM_BLOCKS))
        return;
...
RiDeformationV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiDeformation", VALID_XFORM_BLOCKS))
        return;
...
RiDisplacementV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiDisplacement", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiCoordinateSystem(RtToken space) {
    if (check("RiCoordinateSystem", VALID_XFORM_BLOCKS))
        return;
...
RiCoordSysTransform(RtToken space) {
    if (check("RiCoordSysTransform", VALID_XFORM_BLOCKS))
        return;
...
RiTransformPoints(RtToken fromspace, RtToken tospace, RtInt npoints, RtPoint *points) {
    if (check("RiTransformPoints", RENDERMAN_ALL_BLOCKS))
        return NULL;
...
RiTransformBegin(void) {
    if (check("RiTransformBegin", VALID_XFORM_BLOCKS))
        return;
...
EXTERN(RtVoid)
RiTransformEnd(void) {
    if (check("RiTransformEnd", RENDERMAN_XFORM_BLOCK))
        return;
...
RiAttributeV(const char *name, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiAttribute", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiPolygonV(RtInt nvertices, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiPolygon", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiGeneralPolygonV(RtInt nloops, RtInt *nverts, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiGeneralPolygon", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiPointsPolygonsV(RtInt npolys, RtInt *nverts, RtInt *verts, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiPointsPolygons", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiPointsGeneralPolygonsV(RtInt npolys, RtInt *nloops, RtInt *nverts, RtInt *verts, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiPointsGeneralPolygons", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiBasis(RtBasis ubasis, RtInt ustep, RtBasis vbasis, RtInt vstep) {
    if (check("RiBasis", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiPatchV(RtToken type, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiPatch", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiPatchMeshV(RtToken type, RtInt nu, RtToken uwrap, RtInt nv, RtToken vwrap, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiPatchMesh", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiNuPatchV(RtInt nu, RtInt uorder, RtFloat *uknot, RtFloat umin, RtFloat umax, RtInt nv, RtInt vorder, RtFloat *vknot, RtFloat vmin, RtFloat vmax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiNuPatch", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiTrimCurve(RtInt nloops, RtInt *ncurves, RtInt *order, RtFloat *knot, RtFloat *amin, RtFloat *amax, RtInt *n, RtFloat *u, RtFloat *v, RtFloat *w) {
    ...
    if (check("RiTrimCurve", VALID_ATTRIBUTE_BLOCKS))
        return;
...
RiSphereV(RtFloat radius, RtFloat zmin, RtFloat zmax, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiSphere", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiConeV(RtFloat height, RtFloat radius, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiCone", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiCylinderV(RtFloat radius, RtFloat zmin, RtFloat zmax, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiCylinder", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiHyperboloidV(RtPoint point1, RtPoint point2, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiHyperboloid", VALID_PRIMITIVE_BLOCKS))
        return;
...
EXTERN(RtVoid)
RiParaboloidV(RtFloat radius, RtFloat zmin, RtFloat zmax, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiParaboloid", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiDiskV(RtFloat height, RtFloat radius, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiDisk", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiTorusV(RtFloat majorrad, RtFloat minorrad, RtFloat phimin, RtFloat phimax, RtFloat thetamax, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiTorus", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiCurvesV(RtToken degree, RtInt ncurves, RtInt nverts[], RtToken wrap, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiCurves", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiProcedural(void *data, RtBound bound, RtVoid (*subdivfunc)(void *, RtFloat), RtVoid (*freefunc)(void *)) {
    if (check("RiProcedural", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiGeometryV(RtToken type, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiGeometry", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiPointsV(RtInt npts, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiPoints", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiSubdivisionMeshV(RtToken scheme, RtInt nfaces, RtInt nvertices[], RtInt vertices[], RtInt ntags, RtToken tags[], RtInt nargs[], RtInt intargs[], RtFloat floatargs[], RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiSubdivisionMesh", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiBlobbyV(RtInt nleaf, RtInt ncode, RtInt code[], RtInt nflt, RtFloat flt[], RtInt nstr, RtString str[], RtInt n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiBlobby", VALID_PRIMITIVE_BLOCKS))
        return;
    ...
    if ((pipe(fdin) != -1) && (pipe(fdout) != -1)) {
        ...
        if ((cpid = fork()) >= 0) {
            ...
            if (cpid != 0) {
                ...
                close(fdout[0]); // remap stdin and stdout
...
RiProcDynamicLoad(void *data, RtFloat detail) {
    ...
    if (module != NULL) {
        ...
        if (ConvertParameter != NULL)
            blindData = ConvertParameter(delayed->helper);
        if (Subdivide != NULL)
            Subdivide(blindData, detail);
        if (Free != NULL)
            Free(blindData);
...
RiObjectBegin(void) {
    if (check("RiObjectBegin", RENDERMAN_BLOCK | RENDERMAN_FRAME_BLOCK | RENDERMAN_WORLD_BLOCK | RENDERMAN_ATTRIBUTE_BLOCK | RENDERMAN_XFORM_BLOCK | RENDERMAN_SOLID_PRIMITIVE_BLOCK | RENDERMAN_RESOURCE_BLOCK | RENDERMAN_ARCHIVE_BLOCK))
        return NULL;
...
EXTERN(RtVoid)
RiObjectEnd(void) {
    if (check("RiObjectBegin", RENDERMAN_OBJECT_BLOCK))
        return;
...
RiObjectInstance(RtObjectHandle handle) {
    if (check("RiObjectInstance", VALID_PRIMITIVE_BLOCKS))
        return;
...
RiMotionBeginV(RtInt N, RtFloat times[]) {
    if (check("RiMotionBegin", RENDERMAN_BLOCK | RENDERMAN_FRAME_BLOCK | RENDERMAN_WORLD_BLOCK | RENDERMAN_ATTRIBUTE_BLOCK | RENDERMAN_XFORM_BLOCK | RENDERMAN_SOLID_PRIMITIVE_BLOCK | RENDERMAN_RESOURCE_BLOCK | RENDERMAN_ARCHIVE_BLOCK))
        return;
...
EXTERN(RtVoid)
RiMotionEnd(void) {
    if (check("RiMotionEnd", RENDERMAN_MOTION_BLOCK))
        return;
...
RiMakeTextureV(const char *pic, const char *tex, RtToken swrap, RtToken twrap, RtFilterFunc filterfunc, RtFloat swidth, RtFloat twidth, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiMakeTexture", RENDERMAN_ALL_BLOCKS))
        return;
...
RiMakeBumpV(const char *pic, const char *tex, RtToken swrap, RtToken twrap, RtFilterFunc filterfunc, RtFloat swidth, RtFloat twidth, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiMakeBump", RENDERMAN_ALL_BLOCKS))
        return;
...
RiMakeLatLongEnvironmentV(const char *pic, const char *tex, RtFilterFunc filterfunc, RtFloat swidth, RtFloat twidth, RtInt n, RtToken tokens[], RtPointer params[]) {
    if (check("RiMakeLatLongEnvironment", RENDERMAN_ALL_BLOCKS))
        return;
...
RiMakeCubeFaceEnvironmentV(const char *px, const char *nx, const char *py, const char *ny, const char *pz, const char *nz, const char *tex, RtFloat fov, RtFilterFunc filterfunc, RtFloat swidth, RtFloat twidth, RtInt n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiMakeCubeFaceEnvironmentV", RENDERMAN_ALL_BLOCKS))
        return;
...
RiMakeShadowV(const char *pic, const char *tex, RtInt n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiMakeShadow", RENDERMAN_ALL_BLOCKS))
        return;
...
RiMakeBrickMapV(int nb, const char **src, const char *dest, RtInt n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiMakeBrickMap", RENDERMAN_ALL_BLOCKS))
        return;
...
RiResourceV(RtToken handle, RtToken type, RtInt n, RtToken tokens[], RtPointer parms[]) {
    if (check("RiResource", VALID_ATTRIBUTE_BLOCKS | VALID_XFORM_BLOCKS))
        return;
...
RiResourceBegin(void) {
    if (check("RiResourceBegin", VALID_ATTRIBUTE_BLOCKS | VALID_XFORM_BLOCKS))
        return;
...
EXTERN(RtVoid)
RiResourceEnd(void) {
    if (check("RiResourceEnd", RENDERMAN_RESOURCE_BLOCK))
        return;
...
RiArchiveBeginV(RtToken name, RtInt n, RtToken tokens[], RtPointer parms[]) {
    ...
    if (check("RiArchiveBegin", RENDERMAN_ALL_BLOCKS))
        return NULL;
...
RiArchiveEnd(void) {
    ...
    if (check("RiArchiveEnd", RENDERMAN_ARCHIVE_BLOCK))
        return;
...
RiReadArchiveV(RtString filename, RtArchiveCallback callback, int n, RtToken tokens[], RtPointer params[]) {
    ...
    if (check("RiReadArchive", RENDERMAN_ALL_BLOCKS))
        return;
