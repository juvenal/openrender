 ...
 *
 * Description:
 *   This file implements the functionality for shader.
 *
 * Authors:
...
CShader::~CShader() {
    ...
    while ((cParameter = parameters) != NULL) {
        ...
        if (cParameter->defaultValue != NULL)
            delete[] (float *)cParameter->defaultValue;
...
void CShaderInstance::createCategories() {
    ...
    if (getParameter("__category", &categoryString, NULL, NULL) == TRUE) {
        ...
        do {
            cCat = token(&tmp, ",\t ");
            if (*cCat != '\0')
                categories[i++] = CRenderer::getGlobalID(cCat);
        } while (tmp != NULL);
...
CProgrammableShaderInstance::~CProgrammableShaderInstance() {
    ...
    while ((cParameter = parameters) != NULL) {
        ...
        // Delete the default values
        if (cParameter->defaultValue != NULL) {
            if (cParameter->type == TYPE_STRING)
                delete[] (char **)cParameter->defaultValue;
            else
                delete[] (float *)cParameter->defaultValue;
        }
...
float **CProgrammableShaderInstance::prepare(CMemPage *&namedMemory, float **varying, int numVertices) {
    ...
    for (totalVaryingSize = 0, i = 0; i < numVariables; i++) {
        if (varyingSizes[i] < 0)
            totalVaryingSize += -varyingSizes[i];
        else
            totalVaryingSize += varyingSizes[i] * numVertices * 3;
    }
    ...
    for (i = 0; i < numVariables; i++) {
        ...
        data = align64Ptr(data);

        locals[i] = (float *)data;
        if (varyingSizes[i] < 0)
            data += -varyingSizes[i];
        else
            data += varyingSizes[i] * numVertices * 3;
    }
