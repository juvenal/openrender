 ...
 *
 * Description:
 *   This file implements the functionality for reyes.
 *
 * Authors:
...
CReyes::CReyes(int thread) : CShadingContext(thread) {
    ...
    if (CRenderer::numExtraSamples > 0)
        extraPrimitiveFlags |= RASTER_EXTRASAMPLES;
    if (CRenderer::aperture != 0)
        extraPrimitiveFlags |= RASTER_FOCALBLUR;
...
void CReyes::drawObject(CObject *object) {
    ...
    if (bmin[COMP_Z] > CRenderer::clipMax) {
        return;
    }
    ...
    // Trivial reject
    if (xmin > CRenderer::sampleClipRight)
        return;
    if (ymin > CRenderer::sampleClipBottom)
        return;
    if (xmax < CRenderer::sampleClipLeft)
        return;
...
void CReyes::shadeGrid(CRasterGrid *grid, int Ponly) {
    ...
    if (grid->dim == SHADING_0D) {
        ...
        if (Ponly) {
            ...
            if ((CRenderer::flags & OPTIONS_FLAGS_SAMPLEMOTION) && (object->moving()))
    ...
    if (flags & RASTER_POINT) {
        ...
        for (j = 0; j < vdiv; j++) {
            for (i = 0; i < udiv; i++, bounds += 4) {
                ...
                P += numVertexSamples;
                if (P[COMP_X] < xbound[0])
                    xbound[0] = P[COMP_X];
                else if (P[COMP_X] > xbound[1])
                    xbound[1] = P[COMP_X];
                if (P[COMP_Y] < ybound[0])
                    ybound[0] = P[COMP_Y];
                else if (P[COMP_Y] > ybound[1])
                    ybound[1] = P[COMP_Y];
