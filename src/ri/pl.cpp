 ...
 *
 * Description:
 *   This file implements the functionality for pl.
 *
 * Authors:
...
class CUniformParameter : public CParameter {
    public:
        ...
        void dispatch(int numVertices, float **varying, float ***locals) {
            ...
            if (variable->type == TYPE_STRING) {
                if (variable->storage == STORAGE_GLOBAL)
                    dests = (char **)varying[variable->entry];
                ...
                if (variable->storage == STORAGE_GLOBAL)
                    ...
                    destf = locals[variable->accessor][variable->entry];
                else
                    destf = NULL;
            }
            ...
            if (next != NULL)
                next->dispatch(numVertices, varying, locals);
        }
        ...
        void dispatch(int start, int numVertices, float **varying, float ***locals) {
            ...
            if (next != NULL)
                next->dispatch(start, numVertices, varying, locals);
        }
        ...
        CParameter *clone(CAttributes *a) {
            ...
            if (variable->storage == STORAGE_GLOBAL)
                cUniform = new CUniformParameter(variable);
            else
                cUniform = new CUniformParameter(a->findParameter(variable->name));
...
class CVaryingParameter : public CParameter {
    public:
        ...
        void dispatch(int numVertices, float **varying, float ***locals) {
            ...
            if (variable->storage == STORAGE_GLOBAL)
                dest = varying[variable->entry];
            else if (locals[variable->accessor] != NULL)
                dest = locals[variable->accessor][variable->entry];
            else
                dest = NULL;
        ...
        CParameter *clone(CAttributes *a) {
            ...
            if (variable->storage == STORAGE_GLOBAL)
                cVarying = new CVaryingParameter(variable);
            else
                cVarying = new CVaryingParameter(a->findParameter(variable->name));

            cVarying->data = new float[variable->numFloats * 4];
            memcpy(cVarying->data, data, variable->numFloats * 4 * sizeof(float));

            if (next != NULL)
                cVarying->next = next->clone(a);
...
class CVarying3Parameter : public CParameter {
    public:
        ...
        CParameter *clone(CAttributes *a) {
            ...
            if (variable->storage == STORAGE_GLOBAL)
                cVarying = new CVarying3Parameter(variable);
            else
                cVarying = new CVarying3Parameter(a->findParameter(variable->name));
...
class CVarying2Parameter : public CParameter {
    public:
        ...
        CParameter *clone(CAttributes *a) {
            ...
            if (variable->storage == STORAGE_GLOBAL)
                cVarying = new CVarying2Parameter(variable);
            else
                cVarying = new CVarying2Parameter(a->findParameter(variable->name));
...
void CVertexData::dispatch(const float *data, int start, int numVertices, float **varying, float ***locals) {
    ...
    for (i = numVariables; i > 0; i--, dispatch++) {
        ...
        if (dispatch[0]->storage == STORAGE_GLOBAL)
            dest = varying[dispatch[0]->entry] + start * numFloats;
        else if (locals[dispatch[0]->accessor] != NULL)
            dest = locals[dispatch[0]->accessor][dispatch[0]->entry] + start * numFloats;
        else
            dest = NULL;
...
CPl *parseParameterList(int numUniform, int numVertex, int numVarying, int numFaceVarying, int numParams, const char **params, const void **vals, const char *required, int flags, CAttributes *attributes) {
    ...
    for (dataSize = 0, i = 0; i < numParams; i++) {
        ...
        if (cVar->type == TYPE_STRING)
            dataSize += numItems * cVar->numFloats * (sizeof(char *) / sizeof(float));
        else
