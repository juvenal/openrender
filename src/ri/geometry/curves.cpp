 ...
 *
 * Description:
 *   This file implements the functionality for curves.
 *
 * Authors:
...
void CCurve::interpolate(int numVertices, float **varying, float ***locals) const {
    ...
    if (base->parameters != NULL)
        base->parameters->dispatch(numVertices, varying, locals);
...
// CCurve::dice() lives in geometryDispatch.cpp (real body) /
...
// Return Value			:	-
// Comments				:
void CLinearCurve::sample(int start [[maybe_unused]], int numVertices, float **varying, float ***locals, unsigned int &up) const {
    int j, k;
    float *intr, *intrStart;
    CVertexData *variables = base->variables;
    const int vertexSize = variables->vertexSize;
...
CCurveMesh::CCurveMesh(CAttributes *a, CXform *x, CPl *c, int d, int nv, int nc, int *nve, int w) : CObject(a, x) {
    ...
    numCurves = nc;
    degree = d;
    nverts = new int[numCurves];
    memcpy(nverts, nve, sizeof(int) * numCurves);
    wrap = w;
