...
CCSGPolygon *CCSGPolygon::clone() const {
    ...
    for (i = 0; i < vertices.numItems; i++)
        n->vertices.push(vertices.array[i]);
...
static void csgSplitPolygon(CCSGPolygon *poly, const float planeNormal[3], float planeD, float epsilon, CArray<CCSGPolygon *> *coplanarFront, CArray<CCSGPolygon *> *coplanarBack, CArray<CCSGPolygon *> *front, CArray<CCSGPolygon *> *back) {
    int i, numVerts;
    int *types;
    ...
    if (numFront == 0 && numBack == 0) {
        ...
        if (dotvv(poly->planeNormal, planeNormal) > 0)
    ...
    if (numFront == 0 && numCoplanar > 0 && numBack == 0)
        polygonType = CSG_COPLANAR;
    else if (numBack == 0)
        polygonType = CSG_FRONT;
    else if (numFront == 0)
        polygonType = CSG_BACK;
...
void CCSGBSPNode::build(CArray<CCSGPolygon *> *polys, int depth) {
    ...
    if (frontList.numItems > 0) {
        if (!front) {
            front = new CCSGBSPNode;
            front->planeEpsilon = planeEpsilon;
        }
    ...
    if (backList.numItems > 0) {
        if (!back) {
            back = new CCSGBSPNode;
            back->planeEpsilon = planeEpsilon;
        }
...
CArray<CCSGPolygon *> *CCSGBSPNode::clipPolygons(CArray<CCSGPolygon *> *polys) const {
    ...
    for (i = 0; i < coplanarFront.numItems; i++)
        frontList.push(coplanarFront.array[i]);
    for (i = 0; i < coplanarBack.numItems; i++)
        backList.push(coplanarBack.array[i]);

    if (front) {
        CArray<CCSGPolygon *> *clippedFront = front->clipPolygons(&frontList);
        for (i = 0; i < clippedFront->numItems; i++)
            result->push(clippedFront->array[i]);
        delete clippedFront;
    }
    else {
        for (i = 0; i < frontList.numItems; i++)
            result->push(frontList.array[i]);
    }

    if (back) {
        CArray<CCSGPolygon *> *clippedBack = back->clipPolygons(&backList);
        for (i = 0; i < clippedBack->numItems; i++)
            result->push(clippedBack->array[i]);
        delete clippedBack;
    }
    else {
        // No back child: back-side polygons are outside the solid entirely
        // and are discarded (their memory freed).
        for (i = 0; i < backList.numItems; i++)
            delete backList.array[i];
    }
...
void CCSGBSPNode::clipTo(CCSGBSPNode *other) {
    ...
    for (i = 0; i < polygons.numItems; i++)
        ownedInput.push(polygons.array[i]);
    polygons.numItems = 0;

    clipped = other->clipPolygons(&ownedInput);

    for (i = 0; i < clipped->numItems; i++)
        polygons.push(clipped->array[i]);
    delete clipped;
...
void CCSGBSPNode::allPolygons(CArray<CCSGPolygon *> *out) const {
    ...
    for (i = 0; i < polygons.numItems; i++)
        out->push(polygons.array[i]->clone());

    if (front)
        front->allPolygons(out);
    if (back)
        back->allPolygons(out);
}
