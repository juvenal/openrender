...
void tessQuadricSphere(float r, float umax, float vmin, float vmax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
...
void tessQuadricDisk(float r, float z, float umax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
    for (int u = 0; u < NU; u++) {
...
void tessQuadricCone(float r, float height, float umax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
...
void tessQuadricCylinder(float r, float zmin, float zmax, float umax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
    for (int u = 0; u < NU; u++) {
        float t0 = umax * u / NU, t1 = umax * (u + 1) / NU;
...
void tessQuadricParaboloid(float rmax, float zmin, float zmax, float umax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
    if (zmax <= 0.0f)
        return;
    auto pt = [&](int ui, int vi) -> float3 {
        float theta = umax * ui / NU;
...
void tessQuadricHyperboloid(const float p1[3], const float p2[3], float umax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
    auto pt = [&](int ui, int vi) -> float3 {
        float theta = umax * ui / NU;
        float t = (float)vi / NV;
...
void tessQuadricToroid(float rmax, float rmin, float phimin, float phimax, float umax, const float *m, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
    auto pt = [&](int ui, int vi) -> float3 {
        float theta = umax * ui / NU;
