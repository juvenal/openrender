...
void tessPolygon(
    const float *positions, int npoly, const int *nholes, const int *nvertices, const int *indices, const float *xfrom, const float3 &col, std::vector<float3> &verts, std::vector<float3> &cols, AABB &bounds) {
    int loopBase = 0;
