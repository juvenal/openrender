...
// Vertices appended per disc by expandDisk() below -- exposed so a caller expanding many discs
// into the same vectors can reserve() the exact final size once, up front (see
// dataSink.cpp's buildDataScene()); expandDisk() itself deliberately never reserves internally.
inline constexpr int kVertsPerDisk = 60;
