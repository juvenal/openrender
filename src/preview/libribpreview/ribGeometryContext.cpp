...
CRibGeometryContext::~CRibGeometryContext() {
    ...
    for (auto *xf : xformStack_)
        xf->detach();
    for (auto *at : attrStack_)
        at->detach();
    // Release current
...
void CRibGeometryContext::RiClipping(float hither, float yon) {
    if (hither > 0)
        options_.nearPlane = hither;
    if (yon > hither)
        options_.farPlane = yon;
}
...
void CRibGeometryContext::RiPointsPolygonsV(int npolys, int *nverts, int *verts, int n, const char *tokens[], const void *params[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
...
void CRibGeometryContext::RiPointsGeneralPolygonsV(int npolys, int *nloops, int *nverts, int *verts, int n, const char *tokens[], const void *params[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
...
void CRibGeometryContext::RiPatchMeshV(const char *type, int nu, const char *uwrap, int nv, const char *vwrap, int n, const char *tokens[], const void *params[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
...
void CRibGeometryContext::RiNuPatchV(int nu, int uorder, float *uknot, float /*umin*/, float /*umax*/, int nv, int vorder, float *vknot, float /*vmin*/, float /*vmax*/, int n, const char *tokens[], const void *params[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
...
void CRibGeometryContext::RiSphereV(float radius, float zmin, float zmax, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
...
void CRibGeometryContext::RiConeV(float height, float radius, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    emitObject(new CCone(currentAttributes_, currentXform_, nullptr, 0, radius, height, tmax));
}

void CRibGeometryContext::RiCylinderV(float radius, float zmin, float zmax, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    emitObject(new CCylinder(currentAttributes_, currentXform_, nullptr, 0, radius, zmin, zmax, tmax));
}

void CRibGeometryContext::RiParaboloidV(float rmax, float zmin, float zmax, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    emitObject(new CParaboloid(currentAttributes_, currentXform_, nullptr, 0, rmax, zmin, zmax, tmax));
}

void CRibGeometryContext::RiDiskV(float height, float radius, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    emitObject(new CDisk(currentAttributes_, currentXform_, nullptr, 0, radius, height, tmax));
}

void CRibGeometryContext::RiTorusV(float rmajor, float rminor, float phimin, float phimax, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    emitObject(new CToroid(currentAttributes_, currentXform_, nullptr, 0, rmajor, rminor, phimin, phimax, tmax));
}

void CRibGeometryContext::RiHyperboloidV(float *point1, float *point2, float tmax, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    emitObject(new CHyperboloid(currentAttributes_, currentXform_, nullptr, 0, point1, point2, tmax));
}

void CRibGeometryContext::RiCurvesV(const char *d, int ncurves, int nverts[], const char *w, int n, const char *tokens[], const void *params[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
...
void CRibGeometryContext::RiProcedural(void *data, float *bound, void (*subdivfunc)(void *, float), void (*freefunc)(void *)) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
    if (!bound)
        return;
...
static bool lineIsObjectBeginWithName(const char *line, const char *objectName) {
    while (*line == ' ' || *line == '\t')
        line++;
    if (strncmp(line, "ObjectBegin", 11) != 0)
        return false;
    ...
    if (strncmp(p, objectName, nlen) != 0)
        return false;
    p += nlen;
    return (*p == '"' || *p == '\r' || *p == '\n' || *p == '\0');
}
...
void CRibGeometryContext::loadNamedGeometry(const char *filename, const char *objectName) {
    ...
    while (fgets(line, (int)sizeof(line), file)) {
        ...
        if (lineStartsWithToken(line, "ObjectBegin")) {
            depth++;
            fprintf(out, "%s", line);
            continue;
        }
...
void CRibGeometryContext::RiGeometryV(const char *type, int /*n*/, const char * /*tokens*/[], const void * /*params*/[]) {
    if (!shouldProcess(motionBlockDepth_, motionGeomConsumed_))
        return;
