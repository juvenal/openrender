...
struct SessionCtx {
    ...
    bool     quit       = false;
    float   *pixels     = nullptr; // RGBA32F, width*height*numSamples floats
};
...
// Color format macros (same as fbx.cpp)
#define color_argb(r, g, b, a) ((b << 24) | (g << 16) | (r << 8) | a)

static void runX11Window(int sockfd, SessionCtx &ctx) {
        ...
        while (XPending(display) > 0) {
            ...
            case ClientMessage: {
                ...
                if (static_cast<Atom>(d[0]) == WM_DELETE_WINDOW) {
                    // Send QUIT to renderer so it can set disconnected flag
        ...
        if (socketOpen && (pfs[1].revents & (POLLIN | POLLHUP | POLLERR))) {
            ...
            if (!readExact(sockfd, &hdr, sizeof(hdr))) {
                ...
                if (!ctx.done && !ctx.quit) {
                    ...
                    snprintf(ctx.title, sizeof(ctx.title), "Interrupted \xe2\x80\x94 %s", ctx.baseTitle);
                    XChangeProperty(display, xcanvas, _NET_WM_NAME, UTF8_STRING, 8,
                                    PropModeReplace,
                                    reinterpret_cast<const unsigned char *>(ctx.title),
                                    static_cast<int>(strlen(ctx.title)));
            ...
            case FBOpcode::DATA: {
                ...
                const float *src = reinterpret_cast<const float *>(pbuf.data() + sizeof(FBDataPayload));
            ...
            case FBOpcode::DONE:
                ctx.done = true;
                socketOpen = false;
                snprintf(ctx.title, sizeof(ctx.title), "Rendering Complete \xe2\x80\x94 %s", ctx.baseTitle);
                XChangeProperty(display, xcanvas, _NET_WM_NAME, UTF8_STRING, 8,
                                PropModeReplace,
                                reinterpret_cast<const unsigned char *>(ctx.title),
                                static_cast<int>(strlen(ctx.title)));
...
struct WlCtx {
    ...
    bool                 needQuit = false; // window closed by user → send QUIT
};

static WlCtx *g_wlctx = nullptr; // used in static callbacks

// Wayland SHM helpers (same as fbwl.cpp)
...
static struct libdecor_frame_interface decor_frame_iface = { decor_configure, decor_close, decor_commit,
...
static void runWaylandWindow(int sockfd, SessionCtx &ctx) {
    WlCtx w;
    w.width      = ctx.width;
    w.height     = ctx.height;
...

    {
        ...
        while (w.running) {
            ...
            if (!socketOpen && !w.running) break;
        }
    }

    // Send QUIT if user closed window while render was active
...
int main(int argc, char **argv) {
    ...
    // Connect to renderer's socket (renderer is the server, we are the client)
    ...
    // Wait for the renderer to create the socket (up to 5 s)
    int attempts = 0;
    while (connect(sockfd, reinterpret_cast<struct sockaddr *>(&addr), sizeof(addr)) != 0) {
        if (++attempts > 100) {
            fprintf(stderr, "orender-fb-linux: cannot connect to %s: %s\n",
    ...
    FBHeader hdr;
    if (!readExact(sockfd, &hdr, sizeof(hdr)) || hdr.opcode != FBOpcode::START) {
        fprintf(stderr, "orender-fb-linux: expected START packet\n");
        close(sockfd);
        return 1;
    }
    ...
    if (payloadLen > 0 && !readExact(sockfd, startBuf.data(), payloadLen)) {
    ...
    if (isWaylandAvailable()) {
        runWaylandWindow(sockfd, ctx);
    } else {
        runX11Window(sockfd, ctx);
    }
#else
    runX11Window(sockfd, ctx);
#endif
