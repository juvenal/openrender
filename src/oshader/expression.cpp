 ...
 *
 * Description:
 *   This file implements the functionality for expression.
 *
 * Authors:
...
void CTwoExpressions::getCode(FILE *out, CVariable *dest) {
    ...
    first->getCode(out, nullptr);
    second->getCode(out, nullptr);
}
...
int CVectorExpression::value(char *dest) {
    ...
    if (x->value(tmp1) & y->value(tmp2) & z->value(tmp3)) {
        snprintf(dest, TOKENSIZE, " [ %s %s %s ]", tmp1, tmp2, tmp3);
        return TRUE;
    }
...
void CMatrixExpression::getCode(FILE *out, CVariable *dest) {
    ...
    lock(op0, elements[0]);
    assert(elements[0]->type & SLC_FLOAT);
    lock(op1, elements[1]);
    assert(elements[1]->type & SLC_FLOAT);
    lock(op2, elements[2]);
    assert(elements[2]->type & SLC_FLOAT);
    lock(op3, elements[3]);
    assert(elements[3]->type & SLC_FLOAT);
    lock(op4, elements[4]);
    assert(elements[4]->type & SLC_FLOAT);
    lock(op5, elements[5]);
    assert(elements[5]->type & SLC_FLOAT);
    lock(op6, elements[6]);
    assert(elements[6]->type & SLC_FLOAT);
    lock(op7, elements[7]);
    assert(elements[7]->type & SLC_FLOAT);
    lock(op8, elements[8]);
    assert(elements[8]->type & SLC_FLOAT);
    lock(op9, elements[9]);
    assert(elements[9]->type & SLC_FLOAT);
    lock(op10, elements[10]);
    assert(elements[10]->type & SLC_FLOAT);
    lock(op11, elements[11]);
    assert(elements[11]->type & SLC_FLOAT);
    lock(op12, elements[12]);
    assert(elements[12]->type & SLC_FLOAT);
    lock(op13, elements[13]);
    assert(elements[13]->type & SLC_FLOAT);
    lock(op14, elements[14]);
    assert(elements[14]->type & SLC_FLOAT);
    lock(op15, elements[15]);
...
void CArrayExpression::getCode(FILE *out, CVariable *dest) {
    ...
    if ((array->type & SLC_UNIFORM) ^ (type & SLC_UNIFORM)) {
        if (array->type & SLC_FLOAT)
            opcode = opcodeUFFromArray;
        else if (array->type & SLC_VECTOR)
            opcode = opcodeUVFromArray;
        else if (array->type & SLC_MATRIX)
            opcode = opcodeUMFromArray;
        else if (array->type & SLC_STRING)
            opcode = opcodeUSFromArray;
        else {
            assert(FALSE);
        }
    } else {
        if (array->type & SLC_FLOAT)
            opcode = opcodeFFromArray;
        else if (array->type & SLC_VECTOR)
            opcode = opcodeVFromArray;
        else if (array->type & SLC_MATRIX)
            opcode = opcodeMFromArray;
        else if (array->type & SLC_STRING)
            opcode = opcodeSFromArray;
        else {
            assert(FALSE);
        }
    }
...
void CTerminalExpression::getCode(FILE *out, CVariable *dest) {
    ...
    if (type & SLC_FLOAT)
        opcode = opcodeMoveFloatFloat;
    else if (type & SLC_VECTOR)
        opcode = opcodeMoveVectorVector;
    else if (type & SLC_MATRIX)
        opcode = opcodeMoveMatrixMatrix;
    else if (type & SLC_STRING)
        opcode = opcodeMoveStringString;
    else {
        assert(FALSE);
    }
...
int CSysConversionExpression::value(char *dest) {
    ...
    if (first->value(tmp)) {
        snprintf(dest, 2 * TOKENSIZE, "%s %s", system, tmp);
        return TRUE;
    }
...
CFuncallExpression::~CFuncallExpression() {
    ...
    if (arguments != nullptr) {
        while ((cExpression = arguments->pop()) != nullptr)
            delete cExpression;
        delete arguments;
    }
}
...
void CFuncallExpression::getCode(FILE *out, CVariable *dest) {
    ...
    for (cVariable = function->variables->first(); cVariable != nullptr; cVariable = function->variables->next()) {
    ...
    if (arguments != nullptr) {
        ...
        for (cP = function->parameters->first(), cParameter = arguments->first(); cParameter != nullptr; cParameter = arguments->next(), cP = function->parameters->next()) {
    ...
    if (function->initExpression != nullptr)
        function->initExpression->getCode(out, nullptr);

    // Go over the code
    if (function->code != nullptr)
        function->code->getCode(out, nullptr);
...
CBuiltinExpression::CBuiltinExpression(CFunctionPrototype *f, CList<CExpression *> *p) : CExpression(f->functionType) {
    ...
    replacementPrototype = nullptr;

    if (f->nonuniform == TRUE) {
        fa = 0;
    } else {
        for (cEx = p->first(); cEx != nullptr; cEx = p->next()) {
...
static char getExpressionType(CExpression *e) {
    if (e->type & SLC_FLOAT)
        ...
        if (e->type & SLC_VVECTOR)
            return 'v';
        if (e->type & SLC_VNORMAL)
            return 'n';
        if (e->type & SLC_VPOINT)
            return 'p';
        if (e->type & SLC_VCOLOR)
            return 'c';
        else
            return 'v';
    } else if (e->type & SLC_MATRIX)
        return 'm';
    else if (e->type & SLC_STRING)
        return 's';
    else
        return 'o';
}
...
void CBuiltinExpression::getCode(FILE *out, CVariable *dest) {
    ...
    if (replacementPrototype == nullptr) {
    ...
    for (i = 0, cParameter = arguments->first(); cParameter != nullptr;) {
    ...
    if (function->dso == FALSE) {
        out_printf(out, "\t%s\t(\"%s\") ", function->symbolName, usedPrototype);
    } else {
        out_printf(out, "\tDSO %s\t(\"%s\") ", function->symbolName, usedPrototype);
    }

    if (d != nullptr) {
        out_printf(out, " %s ", d->codeName());
    }

    for (i = 0; i < nParameters; i++) {
        out_printf(out, " %s ", op[i]);
    }
...
CConditionalExpression::CConditionalExpression(int type, CExpression *condition, CExpression *first, CExpression *second) : CExpression(type) {
    ...
    if (!(trueExpression->type & SLC_UNIFORM))
        this->type &= ~SLC_UNIFORM;
    if (!(falseExpression->type & SLC_UNIFORM))
        this->type &= ~SLC_UNIFORM;
}
...
CArrayUpdateExpression::CArrayUpdateExpression(CVariable *f, CExpression *i, CExpression *s, const char *opcodeFloat, const char *opcodeVector, const char *opcodeMatrix) : CExpression(f->type) {
    ...
    if (CExpression *arrayAccessor = new CArrayExpression(first, new CTerminalExpression(indexVar))) {
        ...
        if (CExpression *second = getOperation(arrayAccessor, s, opcodeFloat, opcodeVector, opcodeMatrix, nullptr, 0)) {
...
CArrayUpdateExpression::~CArrayUpdateExpression() {
    if (index != nullptr)
        delete index;
    if (arrayAssigner != nullptr)
        delete arrayAssigner;
}
...
CArrayMove::CArrayMove(CVariable *f, CList<CExpression *> *e) : CExpression(f->type & (~SLC_ARRAY)) {
    ...
    while ((cExpression = e->pop()) != nullptr) {
...
CArrayMove::~CArrayMove() {
    ...
    while ((cExpression = items->pop()) != nullptr) {
        delete cExpression;
    }
}
...
void CArrayMove::getCode(FILE *out, CVariable *dest) {
    ...
    for (i = 0, cExpression = items->first(); cExpression != nullptr; cExpression = items->next(), i++) {
...
void CIfThenElse::getCode(FILE *out, CVariable *dest) {
    ...
    out_printf(out, "%s\t%s ", opcodeIf, condition);
    if (second != nullptr) {
        out_printf(out, "%s\n", elseLabel);
    } else {
        out_printf(out, "%s\n", endLabel);
...
void CGatherThenElse::getCode(FILE *out, CVariable *dest) {
    ...
    *cstr = '\0';
    ...
    for (i = 5; i < numParameters; i++) {
        if (vars[i] == nullptr) {
            out_printf(out, "%s ", varNames[i]);
        } else {
            out_printf(out, "%s ", vars[i]->codeName());
        }
    }
    out_printf(out, "\n");
...
void CForLoop::getCode(FILE *out, CVariable *dest) {
    ...
    if (body != nullptr)
        body->getCode(out, nullptr);

    out_printf(out, "%s:\n", contLabel);

    if (update != nullptr) {
        update->getCode(out, nullptr);
    }
...
// Return Value			:	-
// Comments				:
CIlluminationLoop::CIlluminationLoop(CList<CExpression *> *p, CExpression *b) : CExpression(0), category(nullptr), P(nullptr), N(nullptr), angle(nullptr), body(b) {
    // Note that p is reverse

    // Figure out the number of core items
    int numCore;
    for (numCore = 1; numCore < p->numItems; ++numCore) {
        if (p->array[p->numItems - numCore - 1]->type & SLC_STRING)
            break;
    }
...
void CIlluminationLoop::getCode(FILE *out, CVariable *dest) {
    ...
    out_printf(out, "%s\t", opcodeIlluminance);

    if (op1 != nullptr) {
        out_printf(out, " %s ", op1);
    }

    if (op2 != nullptr) {
        out_printf(out, " %s ", op2);
    }

    if (op3 != nullptr) {
        out_printf(out, " %s ", op3);
    }

    //	out_printf(out," %s %s\n",beginLabel,endLabel);
    out_printf(out, " %s %s", beginLabel, endLabel);
    if (op4 != nullptr) {
        out_printf(out, " %s ", op4);
    }
    out_printf(out, "\n");
...
CIlluminateSolar::CIlluminateSolar(const char *op1, const char *op2, CExpression *p, CExpression *n, CExpression *a, CExpression *b) : CExpression(0) {
    ...
    this->P = (p != nullptr ? getConversion(SLC_VECTOR, p) : nullptr);
    this->N = (n != nullptr ? getConversion(SLC_VECTOR, n) : nullptr);
    this->angle = (a != nullptr ? getConversion(SLC_FLOAT, a) : nullptr);
    this->body = b;
}
...
CExpression *getOperation(CExpression *first, const char *opcodeFloat, const char *opcodeVector, const char *opcodeMatrix, const char *opcodeString, int typeOverwrite) {
    ...
    if (first->type & SLC_FLOAT)
        return new CUnaryExpression((typeOverwrite ? typeOverwrite : SLC_FLOAT), opcodeFloat, first);
    else if (first->type & SLC_VECTOR)
        return new CUnaryExpression((typeOverwrite ? typeOverwrite : (SLC_VECTOR | (first->type & SLC_SUB_TYPE_MASK))), opcodeVector, first);
    else if (first->type & SLC_MATRIX)
        return new CUnaryExpression((typeOverwrite ? typeOverwrite : SLC_MATRIX), opcodeMatrix, first);
    else if (first->type & SLC_STRING)
        return new CUnaryExpression((typeOverwrite ? typeOverwrite : SLC_STRING), opcodeString, first);
    else {
        assert(FALSE);
    }
...
CExpression *getConversion(int type, CExpression *first) {
    ...
    if (type & SLC_STRING) {
        ...
        if (first->type & SLC_VECTOR) {
            first->type |= (type & SLC_SUB_TYPE_MASK);
            return first;
        } else if (first->type & SLC_FLOAT)
        ...
        if (first->type & SLC_MATRIX)
            ...
            return new CUnaryExpression(SLC_MATRIX | (first->type & SLC_UNIFORM), opcodeMatrixFromFloat, first);
        else if (first->type & SLC_VECTOR)
...
#define NEWARRAYPARSE
    ...
    while ((cVar = variables->pop()) != nullptr) {
        hasNamedSpace = FALSE;
        hasComplexRef = FALSE;
        ...
        if (cVar->type & SLC_PARAMETER) {
            ...
            if (expressions->numItems == cPar->numItems) {
                ...
                snprintf(tmp2, expressions->numItems * 128, "[ ");
        ...
        if (pExpression == nullptr)
            pExpression = cExpression;
        else
            pExpression = new CTwoExpressions(cExpression, pExpression);
    }
