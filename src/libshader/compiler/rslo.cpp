...
CFunction *CFunction::getFunction(const char *name, CList<CExpression *> *args, int returnType) {
    ...
    for (cFun = functions->first(); cFun != nullptr; cFun = functions->next()) {
        ...
        // Check names first
        if (strcmp(name, cFun->symbolName) != 0)
            continue;
    ...
    for (cFun = functions->first(); cFun != nullptr; cFun = functions->next()) {
        ...
        for (cPar = cFun->parameters->first(), cArg = args->first(); cPar != nullptr; cPar = cFun->parameters->next(), cArg = args->next()) {
            ...
            if ((cPar->type & SLC_VECTOR) && (cArg->type & SLC_FLOAT))
                continue;
            if ((cPar->type & SLC_MATRIX) && (cArg->type & SLC_FLOAT))
                continue;
            if ((cPar->type & SLC_MATRIX) && (cArg->type & SLC_VECTOR))
                continue;
            break;
        }
        ...
        if (cPar == nullptr) {
            if (!(returnType & SLC_NONE)) {
                ...
                if ((returnType & SLC_VECTOR) & (cFun->returnValue->type & SLC_FLOAT))
                    return cFun;
                if ((returnType & SLC_MATRIX) & (cFun->returnValue->type & SLC_FLOAT))
                    return cFun;
...
CScriptContext::CScriptContext(int s) {
    ...
    addBuiltInFunction("urandom", "f=", 0, TRUE);
    addBuiltInFunction("urandom", "v=", 0, TRUE);
    addBuiltInFunction("urandom", "n=", 0, TRUE);
    addBuiltInFunction("urandom", "p=", 0, TRUE);
    addBuiltInFunction("urandom", "c=", 0, TRUE);
