...
void riThread(void *w) {
    ...
    // Extract SOCKET from pointer storage (64-bit safe)
    SOCKET peer_socket = static_cast<SOCKET>(reinterpret_cast<uintptr_t>(buffer[0].pointer));
    ...
    char *source = strdup(buffer[1].string);
    char *pstart = source;
    char *pend;
    do {
        pend = strchr(pstart, ':');
...
void rndrd(int port) {
    SOCKET sock;
    struct sockaddr_in me;
    T64 buffer[BUFFERSIZE]; // Changed to T64 to support 64-bit SOCKET values
    int running = TRUE;
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "-help") == 0 || strcmp(argv[i], "--help") == 0) {
            ...
            if (i >= argc) {
                if (silent == FALSE)
                    fprintf(stderr, "Was expecting client port\n");
                exit(1);
            }
            ...
            if (i >= argc) {
                if (silent == FALSE)
                    fprintf(stderr, "Was expecting the frame range\n");
                exit(1);
            }
    ...
    if (localChildren != 0) {
        ...
        do {
            ...
            if (osFileExists(pstart) == FALSE) {
                if (silent == FALSE)
                    fprintf(stderr, "Cannot find RIB file '%s'\n", pstart);
                free(source);
                exit(1);
            }
            if (pend)
        ...
        if (runLocalServers(localChildren, source, managerString) == FALSE) {
            if (silent == FALSE)
                fprintf(stderr, "Failed to launch subprocesses\n");
            free(source);
            exit(1);
        }
    }
