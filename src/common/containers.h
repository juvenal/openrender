 ...
 *
 * Description:
 *   This file defines various miscelaneous data structures.
 *
 * Authors:
...
// Comments				:
template <class keyType, class valType>
class CHash : public CDictionary<keyType, valType> {
...
class CTrie : public CDictionary<const char *, valType> {
    ...
    public:
        ...
        int find(const char *key, valType &val) const {
            ...
            for (cNode = root->pointers[(unsigned char)(*key)];; cNode = cNode->pointers[(unsigned char)(*key)]) {
                if (cNode == NULL)
                    return FALSE; // The item not found
    ...
    public:
        ...
        int sanityCheck() {
            ...
            for (i = 0; i < numItems / 2; i++) {
                int j = i * 2;
                if (allItems[j].priority < allItems[i].priority)
                    return FALSE;
                j++;
                if (j < numItems)
                    if (allItems[j].priority < allItems[i].priority)
...
class CList : public CArray<T> {
    public:
        ...
        T next() {
            ...
            if (currentItem == CArray<T>::numItems)
                return 0;
