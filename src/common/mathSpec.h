 ...
 *
 * Description:
 *   This file defines the interface for mathSpec.
 *
 * Authors:
...
inline void mulmm(SCALAR_TYPE *result, const SCALAR_TYPE *s1, const SCALAR_TYPE *s2) {
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++) {
            ...
            for (k = 0; k < 4; k++)
                dest += s1[element(i, k)] * s2[element(k, j)];
...
inline int intersectBox(const SCALAR_TYPE *bmin1, const SCALAR_TYPE *bmax1, const SCALAR_TYPE *bmin2, const SCALAR_TYPE *bmax2) {
    ...
    for (i = 0; i < 3; i++) {
        if ((bmin1[i] > bmax2[i]) || (bmax1[i] < bmin2[i]))
            return FALSE;
    }
...
inline void makeRotation(SCALAR_TYPE *M) {
    ...
    while (TRUE) {
        ...
        if ((x * x + y * y + z * z) < (SCALAR_TYPE)0.000001)
            break;
    }
