 ...
 *
 * Description:
 *   This file implements the functionality for os.
 *
 * Authors:
...
#ifdef _WINDOWS
    if (cModule != NULL)
        FreeLibrary((HMODULE)cModule);
#else
    if (cModule != NULL)
        dlclose((void *)cModule);
#endif
}
...
void osEnumerate(const char *name, int (*callback)(const char *, void *), void *userData) {
    ...
    if ((hFile = _findfirst(name, &c_file)) == -1L) {
        ...
        if (callback(tmp, userData) == TRUE) {
            while (_findnext(hFile, &c_file) == 0) {
                strcpy(tmpp, c_file.name);
                if (callback(tmp, userData) == FALSE)
                    break;
            }
        }
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
        if (callback(globbuf.gl_pathv[i], userData) == FALSE)
            break;
    }
