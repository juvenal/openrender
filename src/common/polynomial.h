 ...
 *
 * Description:
 *   This file defines the interface for polynomial.
 *
 * Authors:
...
//
////////////////////////////////////////////////////////////////////////////
#define cbrt(x) ((x) > 0.0 ? pow(x, 1.0 / 3.0) : ((x) < 0.0 ? -pow(-x, 1.0 / 3.0) : 0.0))

template <class T>
inline int solveQuadric(T a, T b, T c, T *r) {
...
template <class T>
inline int solveCubic(T c[4], T s[3]) {
    int i, num;
    double sub;
    double A, B, C;
    double sq_A, p, q;
...
template <class T>
inline int solveQuartic(T c[5], T s[4]) {
    double coeffs[4];
    double z, u, v, sub;
    double A, B, C, D;
    double sq_A, p, q, r;
