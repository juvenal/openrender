 ...
 *
 * Description:
 *   This file implements the functionality for file_tiff.
 *
 * Authors:
...
// Comments				:
CFileFramebufferTIFF::CFileFramebufferTIFF(const char *name, int width, int height, int numSamples, const char *samples, TDisplayParameterFunction findParameter) : CFileFramebuffer() {
    int i;
    float *tmp;
    float worldToNDC[16];
    ...
    if (tiffcompression == COMPRESSION_LZW || tiffcompression == COMPRESSION_DEFLATE || tiffcompression == COMPRESSION_ADOBE_DEFLATE || tiffcompression == COMPRESSION_PIXARLOG) {
