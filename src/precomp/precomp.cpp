 ...
 *
 * Description:
 *   This file implements the functionality for precomp.
 *
 * Authors:
...
// Comments				:	n has to be a power of 2
template <class T>
static void print(FILE *out, T *a, int n, const char *outName, const char *outType) {
    int i;
...
int precomputeNoiseData() {
    ...
    for (i = 0; i < NOISE_PPERM_SIZE; i++)
        rN[i] = (i + (float)(rand() / (float)RAND_MAX)) / NOISE_PPERM_SIZE;
...
void ouputStochasticFuntionName(FILE *out, unsigned int i) {
    ...
    if (i & RASTER_MOVING)
        fprintf(out, "Moving");
    if (i & RASTER_TRANSPARENT)
        fprintf(out, "Transparent");
    if (i & RASTER_FOCALBLUR)
        fprintf(out, "DepthBlur");
    if (i & RASTER_EXTRASAMPLES)
        fprintf(out, "ExtraSamples");
    if (i & RASTER_MATTE)
        fprintf(out, "Matte");
    if (i & RASTER_LOD)
        fprintf(out, "LOD");
    if (i & RASTER_UNDERCULL)
        fprintf(out, "Undercull");
    if (i & RASTER_XTREME)
        fprintf(out, "Xtreme");
}
...
int precomputeStochasticPrimitivesH() {
    ...
    for (int j = 0; j < 2; j++) {
        if (j)
            fprintf(out, "#ifdef DEFINE_STOCHASTIC_FUNPROTOS\n");
        else
            fprintf(out, "#ifdef DEFINE_STOCHASTIC_FUNCTIONS\n");
        ...
        for (i = 0; i <= caseEnumeration; i++) {
            ...
            if (i & RASTER_MOVING)
                fprintf(out, "\t#define STOCHASTIC_MOVING\n");
            if (i & RASTER_TRANSPARENT)
                fprintf(out, "\t#define STOCHASTIC_TRANSPARENT\n");
            if (i & RASTER_UNSHADED)
                fprintf(out, "\t#define STOCHASTIC_UNSHADED\n");
            if (i & RASTER_FOCALBLUR)
                fprintf(out, "\t#define STOCHASTIC_FOCAL_BLUR\n");
            if (i & RASTER_EXTRASAMPLES)
                fprintf(out, "\t#define STOCHASTIC_EXTRA_SAMPLES\n");
            if (i & RASTER_MATTE)
                fprintf(out, "\t#define STOCHASTIC_MATTE\n");
            if (i & RASTER_LOD)
                fprintf(out, "\t#define STOCHASTIC_LOD\n");
            if (i & RASTER_UNDERCULL)
                fprintf(out, "\t#define STOCHASTIC_UNDERCULL\n");
            if (i & RASTER_XTREME)
                fprintf(out, "\t#define STOCHASTIC_XTREME\n");
            ...
            if (i & RASTER_MOVING)
                fprintf(out, "\t#undef STOCHASTIC_MOVING\n");
            if (i & RASTER_TRANSPARENT)
                fprintf(out, "\t#undef STOCHASTIC_TRANSPARENT\n");
            if (i & RASTER_UNSHADED)
                fprintf(out, "\t#undef STOCHASTIC_UNSHADED\n");
            if (i & RASTER_FOCALBLUR)
                fprintf(out, "\t#undef STOCHASTIC_FOCAL_BLUR\n");
            if (i & RASTER_EXTRASAMPLES)
                fprintf(out, "\t#undef STOCHASTIC_EXTRA_SAMPLES\n");
            if (i & RASTER_MATTE)
                fprintf(out, "\t#undef STOCHASTIC_MATTE\n");
            if (i & RASTER_LOD)
                fprintf(out, "\t#undef STOCHASTIC_LOD\n");
            if (i & RASTER_UNDERCULL)
                fprintf(out, "\t#undef STOCHASTIC_UNDERCULL\n");
            if (i & RASTER_XTREME)
                fprintf(out, "\t#undef STOCHASTIC_XTREME\n");
