...
- (2026-08-10 – 2026-08-13) Added full RISpec subdivision surface support: `RiHierarchicalSubdivisionMesh` (grammar, RI entry point, renderer, RIB round-trip, `orender-wire` preview, Lua binding), the `facevaryinginterpolateboundary`/`facevaryingpropagatecorners`/`creasemethod` subdivision tags, and the Loop subdivision scheme (`subdivisionLoop.{h,cpp}`) alongside Catmull-Clark. Fixed a facevarying pointer-collapse bug causing per-corner UV-seam data loss on shared vertices. Extended visual regression coverage to 75 scenes (from 33) plus 25 parity scenes, spanning REYES, raytrace, and photon hiders. See [SUBDIVISION_SURFACES.md](DEVNOTES_DETAILS/SUBDIVISION_SURFACES.md) (branch `010-full-subdivision-support`, pending commit — no SHA yet).
