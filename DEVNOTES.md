...
| RISpec 3.2 gaps | Re-audited against the source 2026-08-29 and restructured around the spec's own §1.1.2 Advanced Capabilities list. **7 of 11 implemented, 2 partial, 2 absent.** Partial: volume shading (`RiAtmosphere` executes; `RiInterior`/`RiExterior` are stored and never run — the old "blocked on missing CSG" rationale was never the reason and is now doubly false) and area light sources (`RiAreaLightSourceV` is `RiLightSourceV` plus a handle; block geometry is not bound to the light). Absent: special camera projections (perspective/orthographic only) and spectral colors (`RiColorSamples` stores `nColorComps`, nothing reads it — accepted silently). Per-request gaps tracked in a second table: trace subsets, EXR texture input, patch crack stitching still open; blobby, trim curves, subdivision, imager and ray-traced motion blur now closed | [RISPEC_GAPS.md](DEVNOTES_DETAILS/RISPEC_GAPS.md) |
