...
| LLVM JIT shading engine | Complete — `oshader --jit` produces `.slo` LLVM bitcode; `libshader` extraction; JIT runtime; `sloinfo` inspector; covered by the project's 257-scene visual suite (`ctest -L visual`, includes non-JIT scenes too); opcode-coverage parity sweep (`cfrom`/`mfrom`/`ctransform`, matrix arithmetic, comparison/logic, array-move, `gather()`, spec 011) plus builtin-*function*-coverage sweep (`random`/`urandom` via issue #1, then 43 more — 25 originally inventoried plus 18 found mid-implementation — via spec 017); coverage guard now covers the full `OPCODE_*`+`FUNCTION_*` mnemonic universe and is 100% green, though green only proves a dispatch case exists, not that it emits real IR (`printf()`, GitHub #11, is a confirmed no-op exception) — JIT-vs-interpreter wall-clock parity (SC-006) not yet met, root cause documented | [OSHADER_UPDATES.md](DEVNOTES_DETAILS/OSHADER_UPDATES.md) |
...
| RISpec 3.2 gaps | Re-audited against the source 2026-08-29 and restructured around the spec's own §1.1.2 Advanced Capabilities list. **7 of 11 implemented, 2 partial, 2 absent.** Partial: volume shading (`RiAtmosphere` executes; `RiInterior`/`RiExterior` are stored and never run — the old "blocked on missing CSG" rationale was never the reason and is now doubly false) and area light sources (`RiAreaLightSourceV` is `RiLightSourceV` plus a handle; block geometry is not bound to the light). Absent: special camera projections (perspective/orthographic only) and spectral colors (`RiColorSamples` stores `nColorComps`, nothing reads it — accepted silently). Per-request gaps tracked in a second table: trace subsets, patch crack stitching still open; blobby, trim curves, subdivision, imager, ray-traced motion blur, and EXR (plus PNG/RGBE) texture input now closed | [RISPEC_GAPS.md](DEVNOTES_DETAILS/RISPEC_GAPS.md) |
| Multi-format texture source decoding (`otexmake`) | Complete — `otexmake` bakes textures/environment maps/shadow maps from PNG, OpenEXR, and RGBE (Radiance HDR) sources in addition to TIFF, auto-detected by extension, no new CLI flags. New `CImageInput` abstraction (`src/ri/texture/imageInput*.{h,cpp}`) replaces the old TIFF-only `readLayer()` special case at all 5 bake call sites in `texmake.cpp`. Native sample precision preserved (8/16/32-bit); zero colorspace conversion; OpenEXR rejects any multi-part file unconditionally and any channel layout beyond RGB/RGBA/luminance; PNG rejects indexed/palette. TIFF-source bakes verified byte-for-byte identical to pre-change output across all 4 CLI-reachable bake modes. Runtime `texture()`/`environment()` read path and the baked-texture container format are unchanged — this is bake-input only; a separate, later effort covers native runtime reading of non-TIFF sources. `ctest -L image_input` (8 tests: per-decoder round-trips, byte-identical bake regression, cross-format negative cases) plus 3 new cross-hider parity scenes (`Parity_texture-{png,exr,rgbe}`, RGBE's prepared but not registered — see Open Issues) | `specs/018-multi-format-texture-decode/` |
...
| Full subdivision surface support | Complete (US1-US3, US5-US6) — cross-hider (REYES+raytrace+photon) subdivision motion blur, facevarying pointer-collapse fix, `facevaryinginterpolateboundary`/`facevaryingpropagatecorners`/`creasemethod` tags, `RiHierarchicalSubdivisionMesh[V]` (7-layer primitive: grammar, RI entry point, renderer, geometry-layer override resolution, RIB round-trip, preview, Lua binding), Loop scheme as a second scheme alongside Catmull-Clark (mask-based refinement → `CPolygonMesh`, no new hider code); crease-quality reports (US4) investigated and not reproduced (see Open Issues); zero hider-specific subdivision code, grep-verified; 75-scene visual suite (up from 33) + 25-scene parity suite, 100% passing; scenes originally authored to target the (now fully removed) debug/visualization hider per spec.md's Edge Cases are repointed at `orender-wire` by a later feature | [SUBDIVISION_SURFACES.md](DEVNOTES_DETAILS/SUBDIVISION_SURFACES.md), `specs/010-full-subdivision-support/` |
| Solid modeling / CSG (RiSolidBegin/End) | Complete — all three set operations plus `"primitive"`, arbitrarily nested trees (`src/ri/csgTree.{h,cpp}`) resolved by interval boolean classification over ray spans (`src/ri/csgBoolean.{h,cpp}`). Operands are anything that reaches `addObject()`: quadrics, polygon meshes, NURBS, subdivision meshes and blobbies all have scenes. Malformed input (unmatched `SolidEnd`, nesting or a procedural inside `"primitive"`, unknown operation) is diagnosed, not crashed. Replaces the long-standing `CODE_OPTIONAL` stub | [RISPEC_GAPS.md](DEVNOTES_DETAILS/RISPEC_GAPS.md), `specs/013-solid-csg-operations/`, [manual page](docs/site/content/manual/reference/solid-csg-operations.md) |
| Blobby implicit surfaces (RiBlobby) | Complete — all four primitive-field opcodes (1000 constant, 1001 ellipsoid, 1002 segment as a genuine convolution, 1003 repelling ground plane with a context-free depth-file read), all eight combining opcodes with analytic gradients, per-blob primvar blending, `mpoint` reference space (`TYPE_MPOINT`), the `"blobby"/"tolerance"` attribute and `"blobby"/"opcodeorder"` option, RIB round-trip and two-sample motion blur. Surface derived once at `RiBlobby` time by seeded continuation marching tetrahedra into a `CPolygonMesh` through `addObject()` — zero hider-specific code, so CSG operand use and instancing needed no integration at all. Surface threshold *derived* from published figures rather than assumed (0.4; the commonly cited 0.5 is ruled out). 16 unit suites, 27 scenes across three camera hiders (82 visual registrations), 51 parity pairings, 100% passing | [BLOBBY_SURFACES.md](DEVNOTES_DETAILS/BLOBBY_SURFACES.md), `specs/015-blobby-implicit-surfaces/` |
...
- **Multi-Format Texture Source Decoding (`otexmake`)**: `otexmake`'s bake
  pipeline could only ever read TIFF source images — every bake path
  called a shared `readLayer()` helper that hit `libtiff` directly, with
  no abstraction. Introduced `CImageInput`/`CImageInfo`
  (`src/ri/texture/imageInput.h`), a small compiled-in whole-image-decode
  interface (one abstract base, one concrete subclass per format:
  `CTiffImageInput`, `CPngImageInput`, `COpenExrImageInput`,
  `CRgbeImageInput`), with `createImageInput()` dispatching by filename
  suffix. Migrated all 5 `readLayer()` call sites in `texmake.cpp`
  (plain texture, cylindrical/cubic/spherical environment, shadow) onto
  it. `otexmake` can now bake from PNG, OpenEXR, or RGBE (Radiance HDR)
  sources in addition to TIFF, with no new CLI flags, preserving native
  sample precision (8/16/32-bit) and applying zero colorspace conversion.
  OpenEXR sources are single-part only (any multi-part file rejected
  unconditionally, even one with an otherwise-valid single usable part)
  with RGB/RGBA/luminance channels only (`HALF` promoted to float on
  decode); PNG sources reject indexed/palette. `CRgbeImageInput` revives
  `RGBE_ReadHeader`/`RGBE_ReadPixels` (`src/display/rgbe/rgbe.cpp`),
  previously fully implemented but dead code with zero call sites.
  `display/rgbe/rgbe.cpp` is now also compiled directly into
  `ribRender`/`ribVector` (previously only the `rgbe.dsply` display
  module linked it). `HAVE_OPENEXR` is threaded into `src/ri` for the
  first time, gated per-target so `ribVector` (orender-wire's
  lightweight preview library) gains no new OpenEXR dependency even
  though it compiles `texmake.cpp` too. TIFF-source bakes verified
  byte-for-byte identical to pre-change output, across all 4
  CLI-reachable bake modes, against a genuine pre-migration build. The
  baked-texture output format and the runtime `texture()`/`environment()`
  read path are both unchanged — this is bake-input only. See
  `specs/018-multi-format-texture-decode/` and the Open Issues entry
  above for a pre-existing, unrelated bug (`appendLayer()`/
  `TIFFTAG_PHOTOMETRIC`) this work surfaced but did not fix.
...
- **Scene Wireframe Previewer (`orender-wire`)**: Added `orender-wire`, an interactive RIB wireframe viewer shipping as a Metal/AppKit `.app` on macOS and a GTK 4/OpenGL binary on Linux. Built on a new `libribpreview` static library that tessellates all RenderMan primitive types into a flat line-list vertex buffer with per-vertex surface colors. Linux implementation refined for distribution with GTK 4.10 compatibility, terminal detachment, and RPATH resolution (the floor read `gtk4>=4.20` until 2026-09-11; that was intended as 4.2.0 but pkg-config parses it as minor 20, making it stricter than any Ubuntu LTS ships — 4.10 is the real requirement, set by `GtkFileDialog`) (see [VERIFICATION_LINUX_PREVIEW.md](DEVNOTES_DETAILS/VERIFICATION_LINUX_PREVIEW.md)).
...
- **Legacy GUI Removal (`src/gui/`)**: Deleted the unmaintained Qt-based GUI directory. The arcball camera math was re-implemented in Swift (`ArcballCamera.swift`) for macOS and as standalone C++20 inline functions for Linux. (The one file this left behind, `opengl.h`, was itself deleted in a later feature that also removed the last hider still depending on it.)
...
- [ ] `appendLayer()` (`src/ri/texture/texmake.cpp`) never sets
  `TIFFTAG_PHOTOMETRIC` for 3-channel-no-alpha bakes (8-bit or float) —
  causes a libtiff warning storm and a nonzero `orender` exit code when
  rendering with the resulting texture (image content itself is correct).
  Predates and is unrelated to the multi-format-texture-decode work above;
  found because that work was the first to actually render with a freshly
  `otexmake`-baked plain-RGB texture in this repo's test suite. Root cause
  and suggested fix in GitHub issue #18. Blocks registering a cross-hider
  parity scene for RGBE specifically (always exactly 3-channel, no
  single-channel escape hatch the way PNG/OpenEXR's scenes used).
...
- [x] JIT builtin-function coverage gap (GitHub #3, generalizes gotcha #12 below) — completed as `specs/017-jit-builtin-function-coverage/` (branch `017-jit-builtin-function-coverage`). `emitFunction()`'s `kHandledOpcodes[]` allowlist gate silently skipped (zero IR, zero diagnostic) any RSL builtin function it didn't recognize — the same defect class as spec 011's opcode sweep, but for the `FUNCTION_*` family rather than bytecode `OPCODE_*` mnemonics, and independently discovered via `random()`/`urandom()` (GitHub #1, fixed prior to this spec) before the fuller 25-function scope was mapped. All 25 originally-inventoried functions (`visibility`/`transmission`/`trace`/`occlusion`/`indirectdiffuse`/`comp` unblocking `quadlight`/`spherelight`/`ambientocclusion`/`ambientindirect`; `photonmap`/`rayinfo`/`raylabel`/`raydepth`/`ptlined`; `degrees`/`determinant`/`distance`/`match`/`min`/`refract`/`rotate`/`round`/`scale`/`setcomp`/`step`/`translate`/`concat`/`format`) plus 18 more found mid-implementation are now JIT-handled; the `isHandledOpcode()` gate was hardened from silent-skip to a hard `oshader --jit` compile failure naming the unhandled mnemonic; the coverage guard (`LibShader_OpcodeCoverage`) was extended from bytecode-opcode-only to the full `kAllFunctionMnemonics` builtin-function universe (every `DEFFUNC`/`DEFLINKFUNC`/`DEFLIGHTFUNC`/`DEFSHORTFUNC` entry, `scriptFunctions.h → shaderFunctions.h → giFunctions.h`) and is now 100% green.
  **What "100% green" proves, and what it doesn't**: the guard only confirms every reachable mnemonic has an `emitFunction()` dispatch case — never that the case emits meaningful IR. `printf()` (GitHub #11, filed not fixed) is a confirmed counterexample: it has had a dispatch case since before this spec, and that case is `// Silently skip — no per-vertex output` — invisible to the guard and the hardened gate alike. A bounded audit of the rest of the dispatch chain found no other instance of this pattern. Also found and filed, not fixed (pre-existing, orthogonal to this spec's mandate): GitHub #8 (a JIT-wide correctness defect — `collapseArgs`'s uniform fast path substitutes a null tags array whenever an instruction's destination and operands are all uniform, so an `if`-gated assignment to a uniform variable executes unconditionally under JIT regardless of the branch condition; every probe this spec added had to work around it, nothing fixed it), GitHub #9 (`setcomp()`/`comp()` use inconsistent matrix-index formulas — a real transpose mismatch, mirrored not fixed per FR-017), and GitHub #10 (JIT `transform()`'s matrix-argument overload is dispatched as the string-space overload). Also fixed along the way: `.slo` loader (`parseSloShader()`, `rendererFiles.cpp`) constructed `CShader` with the compiled file path instead of the shader's logical name, breaking `shadername()`'s no-arg form and the post-load shader cache key under `--jit` specifically. See `specs/017-jit-builtin-function-coverage/research.md` and `tasks.md`'s T054 note for the full function inventory and per-function verification detail.
...
| [BLOBBY_SURFACES.md](DEVNOTES_DETAILS/BLOBBY_SURFACES.md) | RiBlobby: the derived surface threshold, the opcode 4/5 erratum, the segment convolution, and the extraction walk |
