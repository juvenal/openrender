...
Full implementation of `RiImager` per RI Spec 3.2. The imager executes per pixel tile immediately before display driver dispatch, operating on raw linear float values after pixel filtering and before any quantization or gamma correction. No display driver or shader compiler changes were required.
...
- [x] **Dispatch integration** (`src/ri/rendererDisplay.cpp` `dispatch()`): `if (imagerShader != nullptr)` pre-pass before the channel-copy loop; no display driver interface changes.
