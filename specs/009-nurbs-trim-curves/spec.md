...
- **FR-009**: Because a `NuPatch` mesh is internally split into one child surface (`CNURBSPatch`) per non-degenerate Bezier span, trim loop data and the global knot range MUST be owned by the parent mesh (`CNURBSPatchMesh`) and referenced — not duplicated — by each child span, with each span mapping its local `(u,v)` into the mesh's global parameter range before evaluating trim, so that trimming is correct for spans away from the mesh's origin span.
- **FR-010**: Trim classification MUST be implemented once as a single, shared, hider-agnostic geometric test operating on the `NuPatch` surface's `(u,v)` domain, resolved at the primitive-geometry level rather than inside any one hider's shading or rasterization logic. Every code path that tessellates or samples the surface into discrete vertices MUST consult this same test to exclude vertices outside the retained trim region — including the shared REYES dicing path (`CPatch::dice()`, used by the reyes-family and z-buffer hiders) and the on-demand tessellation path used for ray-surface intersection (`CTesselationPatch`, used by the ray-tracing hider). No hider-specific trim implementation (e.g. a rasterization-only culling step that leaves other hiders untrimmed) is permitted.
- **FR-011**: Trim boundary classification in v1 MUST be a binary accept/reject test per sampled vertex; antialiased or partial-coverage treatment of trim edges is out of scope for this feature.
- **FR-012**: The ray-tracing hider MUST render trimmed `NuPatch` surfaces with the same trimmed result as the reyes-family and z-buffer hiders, by consulting the shared trim test described in FR-010 during its on-demand tessellation of the surface for ray intersection. No hider-specific fallback (untrimmed rendering, a "trimming unsupported" diagnostic, or any other degraded behavior) is permitted for the ray-tracing hider or any other hider that consumes a `NuPatch`'s tessellated output (including the photon-mapping and debug/visualization hiders).
...
- **SC-008**: A trimmed `NuPatch` scene renders the same trimmed silhouette, within the project's existing visual-regression tolerance, whether rendered with the reyes-family hider or the ray-tracing hider, demonstrated by rendering the same trimmed scene under both hiders and comparing outputs.
...
- Trim curve support applies uniformly across every hider, not just the reyes-family bucket-rasterization path. The resolution mechanism is a single, shared, hider-agnostic `(u,v)` inside/outside test resolved at the primitive-geometry level — owned by the `NuPatch` mesh and consulted identically by the REYES dicing path (`CPatch::dice()`) and the ray-tracing hider's on-demand tessellation path (`CTesselationPatch`), both of which already tessellate/sample the same underlying `CSurface` rather than performing hider-specific geometric work. Because trim rejection happens once at this shared layer, every hider that consumes a `NuPatch`'s tessellated output — reyes, z-buffer, ray-tracing, and any other hider built on the same shared tessellation machinery (e.g. the photon-mapping and debug/visualization hiders) — renders trimmed surfaces correctly with no hider-specific implementation and no diagnostic fallback warning.
