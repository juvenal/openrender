...
| Delegation preserved (FR-010) | In the same dump, the callee at each collapsed site is the **same `op_*` symbol** the pre-change dump (T008a) named at that site; only `n` and `tags` differ. A changed callee is a reimplementation, and no image test detects it while the substitute agrees |
