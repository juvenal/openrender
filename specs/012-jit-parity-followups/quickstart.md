...
|---|---|---|
| SC-001 | Stage 1.1 and 1.3 run logs (5 runs each), recorded in `measurements.md` | **Met.** T012/T013: 5/5 pre-fix runs crash (SIGSEGV, exit 139), 100% reproducible. T018/T019: 5/5 post-fix runs pass under both `rslo` and `slo`. |
| SC-002 | `ctest -R <probe>` passes **and** the temporary-revert run in Stage 1.3 shows it failing without the fix | **Met.** `measurements.md` T020a: reverting the fix's stride argument reproduces the original failure (`Visual_sphere-usfroma-reyes-slo`, MaxBlockAvgDiff 116.34 vs. the original 116.81), restoring it passes both scenes 2/2 — confirms the new test would genuinely catch a regression. |
| SC-003 | Three independent before/after pairs — `baselines/us1-before-*.txt`, `baselines/us2-before-*.txt`, `baselines/us3-before-*.txt`, each captured immediately before its own story's change goes live — which for US1 is right after the STOP (mid-stage), and for US2/US3 is just before their end-of-stage rebuild. One per change, not one for the feature; the Stage 0 `base-*.txt` is the feature-level control, not any story's "before" | **Met.** All three pairs captured (T015a, T032a, T044a) and diffed against their respective after-states (T022, T036, T046) — each change verified in isolation against its own control. |
| SC-004 / SC-005 / SC-006 | `baselines/perf-var-1..5.txt` (median = the "before" figure), `baselines/perf-before.txt`, `baselines/perf-after.txt` + the filled density table with every scene in one of exactly two buckets. SC-005 reported, not gated | **SC-004 not met, SC-006 not met** (T037: zero of five "meaningful"-bucket scenes show a confirmed improvement beyond their own run-to-run variance; the paired arrayops/cfrom gap widened fractionally, +0.004 median, well inside noise — the collapse fix is verified correct at the IR level but does not move measured wall-clock time). **SC-005 reported per scene** in T037 — none reach the ≤90%-of-interpreter stretch goal; gate correctly not applied. |
| SC-007 | Full visual suite after each change, compared against the Stage 0.2 noise floor; `git status` shows **zero modified** files under `tests/visual/reference/` and **exactly two added** — the US1 probe image and the FR-006 discrimination image | **Met.** T046: 91/91 visual, 2/2 libshader, zero regressions across all three stories combined. `git status` shows only `??` (added) reference images — the US1 `usfroma` probe and the FR-006 `uniform-conditional` discrimination scene — no modified references. |
| SC-008 | The specific-symbol grep from Stage 3.2, its output recorded verbatim in `measurements.md` | **Met.** T047: `grep -rn "CShadingContext::runLights\|CShadingContext::runCategoryLights\|::runCategoryLights" src/` returns zero matches — no remaining copy of either retired method; the converged `CShadingContext::iterateLights` (`shading.cpp:1508,1512`) is the sole implementation, reached by both backends, confirmed via a dual-shaderformat `matte.sl` render. |
| FR-012 | `DEVNOTES_DETAILS/BUGS.md`, `DEVNOTES_DETAILS/OSHADER_UPDATES.md`, `DEVNOTES.md`, and spec 011's records updated — **and** `docs/site/content/development/releases.md` gains an entry covering the crash fix and the measured performance change. Constitution Principle VII is satisfied by that edit; **no exemption is claimed** | **Met.** All four internal docs updated (T048-T051); `docs/site/content/development/releases.md` gained a "Development build (2026-08-27)" entry naming both the `usfroma` crash fix (with the triggering RSL shape) and the performance investigation outcome (T052). |
