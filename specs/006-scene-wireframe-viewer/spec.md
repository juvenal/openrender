...
- Q: Should the GPU rendering and windowing use a cross-platform library (GLFW/SDL + OpenGL) or platform-native APIs on each OS? → A: Platform-native — Metal + Cocoa on macOS; Vulkan or OpenGL + native display system on Linux. Scene-processing layer (parsing and tessellation) is shared.
...
- The macOS build uses a platform-native rendering pipeline (Metal) and native windowing (Cocoa/AppKit). The Linux build uses a platform-native rendering API (Vulkan or OpenGL) with native display system integration (Wayland or X11). The scene-processing layer — RIB parsing and geometry tessellation — is shared across both platforms.
