...
- [X] T005 [P] Create `src/preview/orender-wire-linux/CMakeLists.txt`: standard executable `orender-wire`, `find_package(OpenGL REQUIRED)`, `pkg_check_modules(GTK4 REQUIRED gtk4>=4.22)`, link against `GTK4_LIBRARIES` and `OpenGL::GL`, `install(TARGETS orender-wire DESTINATION bin)`
