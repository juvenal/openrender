...
- For the interactive irradiance-cache viewer, you may need fltk for your platform.  This is not required to use openRender though.

## Check that you unpacked openrender correctly

If you opted for a binary installation, did you unpack it and place it in the correct location for the OS?

- On OSX, this is `/Applications/Graphics/openRender`
- for Linux, this can be anywhere you like, but the envionment variable`LD_LIBRARY_PATH` needs to be set to the location of the lib files.  In other words, if you put the unpacked contents of the tar file at `/my/path/to/openrender` then you need to do the folllowing `export LD_LIBRARY_PATH=/my/path/to/openrender/lib` for bash or `setenv LD_LIBRARY_PATH /my/path/to/openrender/lib` for tcsh.
...
-rwxr-xr-x   1 501  80     66944 19 Aug 18:16 librslo.0.0.0.dylib
lrwxr-xr-x   1 501  80        18 19 Aug 18:16 librslo.0.dylib -> librslo.0.0.0.dylib
-rw-r--r--   1 501  80    143840 19 Aug 18:16 librslo.a
lrwxr-xr-x   1 501  80        18 19 Aug 18:16 librslo.dylib -> librslo.0.0.0.dylib
-rwxr-xr-x   1 501  80       844 19 Aug 18:16 librslo.la
```

- If the permissions look cookie or there are no symbolic links, make sure you used something like command-line tar to extract ( eg `tar zxvf /path/to/openrender.tgz` ).

## Check that the `ORENDERHOME` environment variable is set

The environment variable `ORENDERHOME` is needed by various parts of openrender to find it's components.  Now there are several ways that openRender can be installed, and which one you did will affect what you should set `ORENDERHOME` to.  The possible methods are - selfcontained and normal.  With each, you can set the target directory.

The OSX binary build is a selfcontained build which needs to be installed at `/Applications/Graphics/openRender`,  Selfcontained builds are designed to live in a single directory.

The linux binary builds are designed to be placed in a single directory somewhere on your system, but be sure to set `LD_LIBRARY_PATH` as noted above.  For these selfcontained builds, set `ORENDERHOME` to be the location you installed to.

- For example, on OSX `export ORENDERHOME=/Applications/Graphics/openRender` for bash or `setenv ORENDERHOME /Applications/Graphics/openRender` for tcsh.
- For a linux build installed to `/my/path/to/openrender`, do `export LD_LIBRARY_PATH=/my/path/to/openrender/` for bash and `setenv LD_LIBRARY_PATH /my/path/to/openrender/` for tcsh.

The normal build type allows you to slot the installation into the standard "FHS" hierachy on a unix / linux installation.  This means that the binaries go to `<prefix>/bin`, libraries to `<prefix>/lib` and so on.  The important difference comes for the data openrender uses - the shaders, display drivers etc.  These will go to `<prefix>/lib/openRender/`.  It is this location which you should use to define `ORENDERHOME`.  This is the default installation type if you do `./configure` then `make install` from source.  The default location to install to is `/usr/local/`.  If you selected another location when you configured (eg. `./configure --prefix=/path/to/prefix` for example ) then the prefix will be prepended on all installation paths.  You can also cause the configure to make a selfcontained build using `./configure --enable-selfcontained --prefix=/path/to/prefix`

- For any build which is not selfcontained, set `ORENDERHOME` to be the `prefix + /lib/openRender`.  If you configured to `/path/to/prefix` for a non-selfcontained build, then use `export ORENDERHOME=/path/to/prefix/lib/openRender` for tcsh or `setenv ORENDERHOME /path/to/prefix/lib/openRender` for tcsh.
- This will be the case for fink builds, which should have `ORENDERHOME` set to `export ORENDERHOME=/sw/lib/openRender` for tcsh or `setenv ORENDERHOME /sw/lib/openRender` for tcsh.

## Errors about libtiff

Libtiff is needed by openRender for reading and writing textures and final images.  Ensure you have a current version set up.  If you get complaints about incompatible versions, then try upgrading.
